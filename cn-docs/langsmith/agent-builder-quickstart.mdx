...
<Steps>
  ...
  <Step title="添加您的密钥 (Add your key)">
    ...
    3. 对于 **Value**，粘贴您在步骤 1 中复制的 API 密钥。
    4. 点击 **Save secret**。
  </Step>
</Steps>
...
<Steps>
  ...
  <Step title="选择模板 (Choose a template)">
    ...
    <Tip>
    如果您不想从模板开始，还有另外两个选择。在 **+ New Agent** 页面：
    - **聊天 (Chat)**：使用聊天界面描述您的智能体，它将帮助您逐步创建。
    - **手动 (Manually)**：选择 **Create manually instead**，在没有预填响应的情况下从头构建您的智能体。
    </Tip>
  </Step>
...
<Steps>
  ...
  <Step title="开始测试聊天 (Start a test chat)">
    1. 在配置页面的右侧面板中，选择 **Test Chat** 选项卡。
    2. 在聊天界面中尝试使用邮件助手，例如：
  ...
  <Step title="智能体开始工作 (Agent starts working)">
    您的智能体将开始工作，并为每个需要您批准的步骤提供 **Continue** 选项。

    <img
    className="block dark:hidden"
    src="/langsmith/images/agent-builder-response.png"
    alt="包含 Gmail 工具批准响应的测试聊天输出视图。"
    />
    ...
    3. 当您测试智能体时，您可以修改指令或添加您可能需要的工具。当您对结果满意时，点击 **Save changes**。
