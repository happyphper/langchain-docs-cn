...
<img
    ...
    alt="LangSmith UI 显示一个名为 ChatOpenAI 的 LLM 调用追踪，包含系统输入、人类输入和 AI 输出。"
/>
...
<Tip> 如果您需要调整组合分数的权重，可以在评估器创建后进行更新。配置了该评估器的所有运行的分数结果都将被更新。 </Tip>
...
<Note> 如果运行中未配置任何组成评估器，则该运行的组合分数将不会被计算。 </Note>
