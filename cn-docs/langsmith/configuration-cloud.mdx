...
创建助手时，您指定一个**图 ID**（图名称，如 `"agent"`）。这将返回一个**助手 ID**（UUID，如 `"62e209ca..."`）。在运行图时，您可以使用任意一个：
...
更新时必须提供**完整**配置。更新端点从头开始创建新版本，不会与先前版本合并。请包含您希望保留的所有配置字段。
</Warning>
...
<Warning>
删除助手将删除其**所有**版本。目前无法删除单个版本。要跳过某个版本，只需将其他版本设置为活动版本即可。
</Warning>
