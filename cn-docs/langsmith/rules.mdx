...
<Note>如果自动化规则匹配跟踪中的任何运行，该跟踪将自动升级为 [延长数据保留期](/langsmith/administration-overview#data-retention-auto-upgrades)。此升级将影响跟踪定价，但确保符合您自动化标准（通常是对分析最有价值的跟踪）的跟踪得以保留以供调查。</Note>
