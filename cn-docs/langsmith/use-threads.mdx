...
<Note>
- **[助手（Assistants）](/langsmith/assistants)** 定义了图如何执行的配置（模型、提示词、工具）。在创建运行时，您可以指定 **图 ID（graph ID）**（例如 `"agent"`）来使用默认助手，或者指定 **助手 ID（assistant ID）**（UUID）来使用特定配置。
- **线程（Threads）** 维护状态和对话历史。
- **运行（Runs）** 将助手和线程结合起来，以特定的配置和状态执行您的图。
</Note>
...
要查看给定线程的当前状态，请使用 @[`get_state`][ThreadsClient.get_state] 方法。这将返回当前值、要执行的下一个节点以及检查点（checkpoint）信息：
...
要查看线程的历史记录，请使用 @[`get_history`][ThreadsClient.get_history] 方法。这将返回线程经历过的每一个状态列表，允许您追踪完整的执行路径：
