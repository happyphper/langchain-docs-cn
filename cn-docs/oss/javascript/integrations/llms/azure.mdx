...
---
<Warning>
**您当前正在查阅的是 Azure OpenAI 文本补全模型的使用文档。最新且最受欢迎的 Azure OpenAI 模型是 [聊天补全模型](/oss/langchain/models)。**

除非您明确在使用 `gpt-3.5-turbo-instruct`，否则您可能正在寻找 [此页面](/oss/integrations/chat/azure/)。
</Warning>

<Info>
**此前，LangChain.js 使用专用的 [Azure OpenAI SDK](https://github.com/Azure/azure-sdk-for-js/tree/main/sdk/openai/openai) 来支持与 Azure OpenAI 的集成。该 SDK 现已弃用，转而采用 OpenAI SDK 中新的 Azure 集成方案。新方案允许在 OpenAI 模型和功能发布当天即可访问，并能在 OpenAI API 和 Azure OpenAI 之间实现无缝切换。**
...
2.  更新您的导入语句，使用来自 `@langchain/openai` 包的新 `AzureOpenAI` 和 @[`AzureChatOpenAI`] 类：
