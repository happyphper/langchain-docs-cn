...
<Note>
**部分结果**：在本教程中，所有选定的智能体必须在合成之前完成。对于更高级的模式，例如您希望处理部分结果或超时，请参阅 [map-reduce指南](/oss/langchain/map-reduce)。
</Note>
...
<Warning>
**有状态路由器增加了复杂性。** 当在不同轮次中路由到不同的智能体时，如果智能体具有不同的语气或提示词，对话可能会感觉不一致。请考虑使用 [交接模式](/oss/langchain/multi-agent/handoffs) 或 [子智能体模式](/oss/langchain/multi-agent/subagents) 代替——两者都为与不同智能体的多轮对话提供了更清晰的语义。
</Warning>
