...
要使 `RemoveMessage` 生效，你需要使用带有 @[`add_messages`] [归约器（reducer）](/oss/langgraph/graph-api#reducers)的状态键。
...
:::js
要从图状态中删除消息，可以使用 `RemoveMessage`。要使 `RemoveMessage` 生效，你需要使用带有 @[`messagesStateReducer`][messagesStateReducer] [归约器（reducer）](/oss/langgraph/graph-api#reducers)的状态键，例如 `MessagesZodState`。
...
<Warning>
    删除消息时，**务必确保**生成的消息历史是有效的。请检查你使用的 LLM 提供商的限制。例如：
