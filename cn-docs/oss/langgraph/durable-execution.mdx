...
:::python
1. 将任何非确定性操作（例如，随机数生成）或具有副作用的操作（例如，文件写入、API 调用）包装在 @[`task`] 内部，以确保当工作流恢复时，这些操作不会针对特定运行重复执行，而是从持久化层检索其结果。更多信息，请参阅[确定性与一致性重放](#determinism-and-consistent-replay)。
:::

:::js
1. 将任何非确定性操作（例如，随机数生成）或具有副作用的操作（例如，文件写入、API 调用）包装在 @[tasks][task] 内部，以确保当工作流恢复时，这些操作不会针对特定运行重复执行，而是从持久化层检索其结果。更多信息，请参阅[确定性与一致性重放](#determinism-and-consistent-replay)。
:::
...
:::python
有关需要避免的常见陷阱示例，请参阅功能 API 中的[常见陷阱](/oss/langgraph/functional-api#common-pitfalls)部分，该部分展示了如何使用**任务**来构建代码以避免这些问题。相同的原则适用于 @[StateGraph (Graph API)][StateGraph]。
:::
...
:::python
*   **暂停和恢复工作流**：使用 @[interrupt][interrupt] 函数在特定点暂停工作流，并使用 @[`Command`] 原语以更新后的状态恢复它。更多细节请参阅[**中断**](/oss/langgraph/interrupts)。
...
:::python
*   如果你正在使用 @[StateGraph (Graph API)][StateGraph]，起始点是执行停止的[**节点**](/oss/langgraph/graph-api#nodes)的开头。
