...
<Tabs>
    ...
    本示例演示了如何使用 @[`BinaryOperatorAggregate`] 通道来实现一个归约器 (reducer)。
...
<Tabs>
    ...
    :::python
    在 [函数式 API (Functional API)](/oss/langgraph/functional-api) 中，您可以使用 @[`@entrypoint`] 来创建 Pregel 应用程序。`entrypoint` 装饰器允许您定义一个接收输入并返回输出的函数。
    ...
    :::js
    在 [函数式 API (Functional API)](/oss/langgraph/functional-api) 中，您可以使用 @[entrypoint][entrypoint] 来创建 Pregel 应用程序。`entrypoint` 装饰器允许您定义一个接收输入并返回输出的函数。
