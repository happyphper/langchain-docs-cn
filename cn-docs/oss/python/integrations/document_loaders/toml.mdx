...
title: TOML
---
>[TOML](https://en.wikipedia.org/wiki/TOML) 是一种用于配置文件的文件格式。它旨在易于读写，并且设计为能够明确地映射到字典。其规范是开源的。`TOML` 已在许多编程语言中实现。名称 `TOML` 是 "Tom's Obvious, Minimal Language" 的首字母缩写，指的是其创建者 Tom Preston-Werner。
