...
<Note>
    **统一 SDK 与包整合**
    ...
    自 `langchain-google-genai` 4.0.0 版本起，该包使用整合后的 [`google-genai`](https://googleapis.github.io/python-genai/) SDK，现在支持 **Gemini 开发者 API 和 Vertex AI** 两种后端。
...
<AccordionGroup>
    <Accordion title="Google Generative AI (Gemini API & Vertex AI)">
        ...
        - **Gemini 开发者 API**：使用 API 密钥快速设置，适合个人开发者和快速原型设计
...
### 聊天模型
<Note>
    **对于 Gemini 模型**，请使用 `langchain-google-genai` 中的 [`ChatGoogleGenerativeAI`](/oss/integrations/chat/google_generative_ai) 而不是 `ChatVertexAI`。它支持 Gemini 开发者 API 和 Vertex AI 两种后端。

    下面的类专注于**Vertex AI 平台服务**，这些服务*不*在整合后的 SDK 中提供。
...
<Columns cols={2}>
    <Card title="ChatVertexAI" icon="comments" href="/oss/integrations/chat/google_vertex_ai" cta="Get started" arrow>
        **已弃用** – 对于 Gemini 模型，请改用 [`ChatGoogleGenerativeAI`](/oss/integrations/chat/google_generative_ai)。
    </Card>
...
<Columns cols={2}>
    ...
    <Card title="VertexAI" icon="i-cursor" href="/oss/integrations/llms/google_vertex_ai" cta="Get started" arrow>
        **已弃用** – 对于 Gemini 模型，请改用 [`GoogleGenerativeAI`](/oss/integrations/llms/google_generative_ai)。
    </Card>
</Columns>
