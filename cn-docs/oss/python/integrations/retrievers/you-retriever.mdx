...
title: You.com
---
>[you.com API](https://api.you.com) 是一套工具集，旨在帮助开发者将大语言模型（LLM）的输出基于其训练数据集中可能未包含的最新、最准确、最相关的信息进行“落地”（grounding）。

## 设置

检索器（retriever）位于 `langchain-community` 包中。
...
```python
# .results 返回解析后的结果，每个代码片段（snippet）都是一个 Document
response = utility.results(query="What is the weather in NY")

# .results 应该为每个 `snippet` 包含一个 Document
print(len(response))
