...
title: AskNews
---
> [AskNews](https://asknews.app) 通过单一自然语言查询，为任何 LLM 注入最新的全球新闻（或历史新闻）。具体来说，AskNews 每天通过翻译、总结、提取实体并将其索引到热和冷向量数据库中来丰富超过 30 万篇文章。AskNews 将这些向量数据库放在一个低延迟的端点上供您使用。当您查询 AskNews 时，您会收到一个经过提示优化的字符串，其中包含所有最相关的丰富信息（例如实体、分类、翻译、摘要）。这意味着您无需管理自己的新闻 RAG（检索增强生成），也无需担心如何以简洁的方式正确地向您的 LLM 传达新闻信息。
> AskNews 也致力于透明度，这就是为什么我们的覆盖范围受到监控，并跨越数百个国家、13 种语言和 5 万个来源进行多样化。如果您想跟踪我们的来源覆盖情况，可以访问我们的[透明度仪表板](https://asknews.app/en/transparency)。
...
我们还需要设置 AskNews API 凭证，这些凭证可以在 [AskNews 控制台](https://my.asknews.app) 获取。
...
我们在这里展示如何将其用作智能体的一部分。我们使用 OpenAI Functions Agent，因此我们需要设置并安装所需的依赖项。我们还将使用 [LangSmith Hub](https://smith.langchain.com/hub) 来拉取提示，因此我们也需要安装它。
