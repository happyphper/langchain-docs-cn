...
# 或者，如果你想从环境变量 BRAVE_SEARCH_API_KEY 中读取 API 密钥，并保持 search_kwargs 为空
# tool = BraveSearch()

# 或者，如果你只想提供 API 密钥，并保持 search_kwargs 为空
# tool = BraveSearch.from_api_key(api_key=api_key)

# 或者，如果你只想提供 search_kwargs，并从 BRAVE_SEARCH_API_KEY 环境变量中读取 API 密钥
