...
title: vlite
---
VLite 是一个简单且极速的向量数据库，允许您使用嵌入（embeddings）以语义方式存储和检索数据。基于 numpy 构建，vlite 是一个轻量级、功能齐全的数据库，可在您的项目中实现 RAG、相似性搜索和嵌入功能。
...
VLite 会为您处理文本的分块和嵌入，您可以通过预先分块文本和/或将分块嵌入到 VLite 数据库中来更改这些参数。
...
VLite 还支持最大边际相关性（Max Marginal Relevance, MMR）搜索，该搜索优化了与查询的相似性以及检索文档之间的多样性。
