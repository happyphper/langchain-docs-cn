...
title: Zep Cloud
---
> 从聊天历史中回忆、理解并提取数据。赋能个性化AI体验。

> [Zep](https://www.getzep.com) 是一个面向AI助手应用的长时记忆服务。
> 借助Zep，您可以为AI助手提供回忆过往对话的能力，无论对话发生在多久以前，
> 同时还能减少幻觉、降低延迟和成本。
...
在下面的示例中，我们使用了Zep的自动嵌入功能，该功能使用低延迟嵌入模型在Zep服务器上自动嵌入文档。

## 注意

- 这些示例使用Zep的异步接口。通过移除方法名前的 `a` 前缀来调用同步接口。
