...
<Steps>
  <Step title="导航到 MCP 服务器设置">
    在 [LangSmith UI](https://smith.langchain.com) 中，选择左下角侧边栏的 **Settings**，然后选择 **MCP Servers** 选项卡。
  </Step>
  ...
  <Step title="配置身份验证">
    ...
    - **Headers**：添加随每个请求发送的键值对。最常见的模式是使用 Authorization 承载令牌：
        - **Key**：`Authorization`
    ...
    - **OAuth 2.1 (Auto)**：对于支持通过动态客户端注册进行 OAuth 的服务器，请选择此项。系统将提示您使用该服务的帐户登录。
    - **OAuth 2.1 (Manual)**：对于支持 OAuth 但需要预先指定客户端 ID/密钥的服务器，请选择此项。在此流程中使用的 OAuth 提供商必须启用 **PKCE**。
...
<Step title="保存并验证">
    点击**保存服务器**。Agent Builder 将自动从您的 MCP 服务器发现可用工具，并使它们在您的智能体（agent）中可用。配置的标头将同时应用于工具发现请求和工具执行请求。
  </Step>
</Steps>
...
<Steps>
  <Step title="创建 GitHub 授权令牌">
    ...
    MCP 服务器需要获得代表您访问 GitHub 的权限。您将使用 **GitHub 个人访问令牌（PAT）** 来实现这一点。

    1.  转到 **GitHub → 设置 → 开发者设置**。
    1.  打开 **个人访问令牌**。
    1.  创建一个 **细粒度令牌**（推荐）。
    ...
    *   **拉取请求**：读取
    ...
    <Tip>请将此令牌视为密码。您可以随时轮换或撤销它，而无需更改您的智能体（agent）。</Tip>
  ...
  <Step title="配置 MCP 服务器">
    ...
    1.  点击 **添加服务器**。
    1.  为 MCP 服务器添加一个 **名称**。
    1.  **URL**：输入您的 MCP 服务器 URL（例如，`https://mcp-github.example.com`）
    1.  添加身份验证标头：
        *   键：`Authorization`
        *   值：`Bearer {{GITHUB_TOKEN}}`
    1.  保存配置。
  </Step>
