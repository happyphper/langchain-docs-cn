...
1.  在提示词中添加 `{{variable_name}}`（对于 `f-string` 格式，每边使用一个花括号；对于 `mustache` 格式，每边使用两个花括号）。![带变量的提示词](/langsmith/images/prompt-with-variable.png)
...
在 Playground 中使用 **[Polly](/langsmith/polly)**，借助 AI 辅助生成工具、创建输出模式并优化你的提示词。
</Callout>
