...
<Warning>
**仅用于开发/测试**。请勿在生产环境中使用 Docker Compose。对于生产部署，请使用 [Kubernetes](/langsmith/kubernetes)。
</Warning>

<Note>
本页描述了如何安装基础的 [LangSmith 平台](/langsmith/self-hosted#langsmith) 以进行本地测试。它**不**包含部署管理功能。更多详情，请查看[自托管选项](/langsmith/self-hosted)。
</Note>
