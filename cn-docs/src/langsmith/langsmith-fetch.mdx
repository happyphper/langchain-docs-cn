...
2.  设置您的项目名称：
    ...
    <Note>
    要获取线程，`langsmith-fetch` **需要** 项目 UUID。因此，请确保设置了 `LANGSMITH_PROJECT` 环境变量，或者在运行 [`langsmith-fetch thread <thread-id>`](#fetch-a-trace-or-thread) 时手动指定 `--project-uuid <project-id>`。
    </Note>
