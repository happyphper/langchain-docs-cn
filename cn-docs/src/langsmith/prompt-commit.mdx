...
<Note>
  理解 LangSmith 的提示提交 Webhook 通常在**工作区级别**触发非常重要。这意味着，如果您的 LangSmith 工作区中*任何*提示被修改并保存了“提示提交”，Webhook 就会触发并发送该提示的更新清单。负载可以通过提示 ID 来识别。您的接收服务器应考虑到这一点进行设计。
</Note>
...
<Accordion title="最小化 FastAPI 服务器代码 ()">
  ...
  *   **配置（`.env`）：** 它期望一个包含您的 `GITHUB_TOKEN`、`GITHUB_REPO_OWNER` 和 `GITHUB_REPO_NAME` 的 `.env` 文件。您还可以自定义 `GITHUB_FILE_PATH`（默认：`LangSmith_prompt_manifest.json`）和 `GITHUB_BRANCH`（默认：`main`）。
