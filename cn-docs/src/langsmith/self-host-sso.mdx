...
<Note>
您可以将[基础身份验证](/langsmith/self-host-basic-auth)安装升级到此模式，但不能升级[无身份验证](/langsmith/authentication-methods#none)安装。要进行升级，只需移除基础身份验证配置，并添加如下所示的必需配置参数。之后，用户将*只能*通过 OAuth 登录。**为了在升级后保持访问权限，您必须能够使用之前通过基础身份验证登录过的电子邮件地址通过 OAuth 登录。**
</Note>
