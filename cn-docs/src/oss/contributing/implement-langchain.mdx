...
<Tabs>
    <Tab title="聊天模型">
        聊天模型是 @[`BaseChatModel`] 类的子类。它们实现了生成聊天补全、处理消息格式和管理模型参数的方法。
    ...
    <Tab title="工具">
        ...
        Tools 类必须继承自 @[`BaseTool`] 基类。此接口包含 3 个属性和 2 个方法，应在子类中实现。
    ...
    <Tab title="向量存储">
        所有向量存储必须继承自 @[`VectorStore`] 基类。此接口包含用于在向量存储中写入、删除和搜索文档的方法。
