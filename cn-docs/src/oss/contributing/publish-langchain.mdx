...
:::python
<Warning>
**重要提示：新的集成应该是独立的包，而不是提交到 LangChain 单仓库的 PR。**

虽然 LangChain 在主仓库中维护了一小部分第一方和高使用率的集成（如 OpenAI、Anthropic 和 Ollama），但**新的集成应该作为单独的 PyPI 包和仓库发布**（例如 `langchain-yourservice`），用户需要与核心 LangChain 包一起安装。你**不应该**提交 PR 来直接将你的集成添加到主 LangChain 仓库。
</Warning>
