...
要使用 @[CharacterTextSplitter] 进行分割，然后使用 `tiktoken` 合并块，请使用其 `.from_tiktoken_encoder()` 方法。请注意，此方法的分割结果可能大于 `tiktoken` 分词器测量的块大小。
...
我们可以使用 `tiktoken` 通过 @[TokenTextSplitter] 来估算使用的令牌数。对于 OpenAI 模型，这可能更准确。
...
<Note>
**[自然语言工具包](https://en.wikipedia.org/wiki/Natural_Language_Toolkit)，或更常见的 [NLTK](https://www.nltk.org/)，是一套用 Python 编程语言编写的用于符号和统计自然语言处理 (NLP) 的库和程序套件。**
</Note>
