...
在使用 LangChain 构建和运行智能体（agent）时，你需要了解其行为细节：它调用了哪些[工具](/oss/langchain/tools)、生成了什么提示（prompt）以及如何做出决策。使用 @[`create_agent`] 构建的 LangChain 智能体自动支持通过 [LangSmith](/langsmith/home) 进行追踪。LangSmith 是一个用于捕获、调试、评估和监控 LLM 应用行为的平台。

:::
:::js

在使用 LangChain 构建和运行智能体（agent）时，你需要了解其行为细节：它调用了哪些[工具](/oss/langchain/tools)、生成了什么提示（prompt）以及如何做出决策。使用 @[`createAgent`] 构建的 LangChain 智能体自动支持通过 [LangSmith](/langsmith/home) 进行追踪。LangSmith 是一个用于捕获、调试、评估和监控 LLM 应用行为的平台。
