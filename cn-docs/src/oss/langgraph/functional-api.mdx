...
<Accordion title="详细解释 (Detailed Explanation)">
...
<Warning>
**序列化 (Serialization)**
入口点的 **输入** 和 **输出** 必须是 JSON 可序列化的，以支持检查点。请参阅 [序列化 (#serialization)](#serialization) 部分了解更多详情。
</Warning>
...
<Accordion title="请求可注入参数 (Requesting Injectable Parameters)">
  ```python
...
:::python
使用 [`@entrypoint`](#entrypoint) 将产生一个 @[`Pregel`][Pregel.stream] 对象，可以使用 `invoke`、`ainvoke`、`stream` 和 `astream` 方法执行。
...
:::python
在 @[interrupt][interrupt] 之后恢复执行可以通过向 @[`Command`] 原语传递 **resume** 值来完成。
...
:::python
@[`entrypoint.final`][entrypoint.final] 是一个特殊的原语，可以从入口点返回，它允许 **解耦 (decoupling)** **保存在检查点中** 的值与 **入口点的返回值**。
...
如果在恢复时未保持执行顺序，则一个 @[`interrupt`] 调用可能会与错误的 `resume` 值匹配，从而导致错误的结果。
