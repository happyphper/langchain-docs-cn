...
当执行恢复时（在您提供请求的输入之后），运行时**会从头开始重新启动整个节点**——它不会从调用 @[`interrupt`] 的确切行恢复。这意味着在 @[`interrupt`] 之前运行的任何代码都会再次执行。因此，在使用中断时，需要遵循一些重要规则以确保其行为符合预期。
...
@[`interrupt`] 通过在调用点抛出特殊异常来暂停执行。如果您将 @[`interrupt`] 调用包装在 try/catch 块中，您将捕获此异常，中断将不会传递回图。
...
* 🔴 不要在裸 try/catch 块中包装 @[`interrupt`] 调用
...
当在节点内调用子图时，父图将从**调用子图并触发 @[`interrupt`] 的节点开头**恢复执行。同样，**子图**也将从调用 @[`interrupt`] 的节点开头恢复。
...
<Note>
    静态中断**不**推荐用于人机协同工作流。请改用 @[`interrupt`] 函数。
</Note>
