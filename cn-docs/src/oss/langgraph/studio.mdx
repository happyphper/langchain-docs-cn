...
<Warning>
Safari 会阻止到 Studio 的 `localhost` 连接。要解决此问题，请使用 `--tunnel` 运行上述命令，通过安全隧道访问 Studio。您需要在 Studio UI 中点击 **Connect to a local server**，手动将隧道 URL 添加到允许的来源中。具体步骤请参阅 [故障排除指南](/langsmith/troubleshooting-studio#safari-connection-issues)。
</Warning>
