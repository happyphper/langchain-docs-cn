...
:::python
请注意，本指南是 LangGraph 特有的，涵盖了具有自定义结构的图（graph）相关场景——如果你是刚开始接触，请查看[此章节](/oss/langchain/test/)，该章节使用了 LangChain 内置的 @[`create_agent`] 函数。
:::

:::js
请注意，本指南是 LangGraph 特有的，涵盖了具有自定义结构的图（graph）相关场景——如果你是刚开始接触，请查看[此章节](/oss/langchain/test/)，该章节使用了 LangChain 内置的 @[`createAgent`] 函数。
:::
...
:::python
1.  使用检查点器（checkpointer）编译你的智能体（用于测试时，内存检查点器 @[`InMemorySaver`] 就足够了）。
