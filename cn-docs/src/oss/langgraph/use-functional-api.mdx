...
<Warning>
**Python < 3.11 下的异步**
如果使用 Python < 3.11 并编写异步代码，使用 @[`get_stream_writer`] 将无效。请直接使用 `StreamWriter` 类。更多详情请参阅 [Python < 3.11 下的异步](/oss/langgraph/streaming#async)。
...
Functional API 支持使用 @[`interrupt`] 函数和 `Command` 原语实现 [人在回路](/oss/langgraph/interrupts) 工作流。
...
[interrupt()](/oss/langgraph/interrupts#pause-using-interrupt) 在任务内部被调用，使人工能够审查和编辑前一个任务的输出。先前任务（本例中为 `step_1`）的结果会被持久化，因此在 @[`interrupt`] 之后不会再次运行。
...
注意，我们在 `step_1` 之后通过 @[`interrupt`] 暂停了。中断提供了恢复运行的指令。要恢复，我们发出一个包含 `human_feedback` 任务期望数据的 [`Command`](/oss/langgraph/interrupts#resuming-interrupts)。
...
给定一个工具调用，我们的函数将 @[`interrupt`] 以等待人工审查。此时我们可以：
...
现在我们可以更新我们的 [入口点](/oss/langgraph/functional-api#entrypoint) 来审查生成的工具调用。如果工具调用被接受或修改，我们像以前一样执行。否则，我们只追加人工提供的 @[`ToolMessage`]。先前任务（本例中是初始模型调用）的结果会被持久化，因此在 @[`interrupt`] 之后不会再次运行。
...
一个使用 Functional API 和 @[`InMemorySaver`] 检查点存储器的简单聊天机器人示例。
