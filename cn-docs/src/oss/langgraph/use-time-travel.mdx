...
:::python
1.  [运行图](#1-run-the-graph)：使用 @[`invoke`][CompiledStateGraph.invoke] 或 @[`stream`][CompiledStateGraph.stream] 方法，结合初始输入运行图。
2.  [识别现有线程中的检查点](#2-identify-a-checkpoint)：使用 @[`get_state_history`] 方法检索特定 `thread_id` 的执行历史，并定位所需的 `checkpoint_id`。
    或者，在您希望执行暂停的节点之前设置一个[中断](/oss/langgraph/interrupts)。然后，您可以找到该中断之前记录的最新检查点。
3.  [更新图状态（可选）](#3-update-the-state-optional)：使用 @[`update_state`] 方法在检查点处修改图的状态，并从替代状态恢复执行。
4.  [从检查点恢复执行](#4-resume-execution-from-the-checkpoint)：使用 `invoke` 或 `stream` 方法，输入为 `None`，配置中包含适当的 `thread_id` 和 `checkpoint_id`。
:::

:::js
1.  [运行图](#1-run-the-graph)：使用 @[`invoke`][CompiledStateGraph.invoke] 或 @[`stream`][CompiledStateGraph.stream] 方法，结合初始输入运行图。
2.  [识别现有线程中的检查点](#2-identify-a-checkpoint)：使用 @[`getStateHistory`] 方法检索特定 `thread_id` 的执行历史，并定位所需的 `checkpoint_id`。
    或者，在您希望执行暂停的节点之前设置一个[断点](/oss/langgraph/interrupts)。然后，您可以找到该断点之前记录的最新检查点。
3.  [更新图状态（可选）](#3-update-the-state-optional)：使用 @[`updateState`] 方法在检查点处修改图的状态，并从替代状态恢复执行。
...
:::python
@[`update_state`] 将创建一个新的检查点。新的检查点将与同一个线程关联，但具有新的检查点 ID。
