...
title: Infino
---
>[Infino](https://github.com/infinohq/infino) 是一个为日志、指标和追踪设计的可扩展遥测存储系统。Infino 既可以作为独立的可观测性解决方案，也可以作为您可观测性技术栈中的存储层。
...
# 创建回调处理器。这将向 Infino 记录延迟、错误、令牌使用情况、提示词以及提示词响应。
...
# 向 OpenAI 模型提问的问题数量。为了在运行此演示时节省 $$，我们限制为一个较小的数字。
num_questions = 10
