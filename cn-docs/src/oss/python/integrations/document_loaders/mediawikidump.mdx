...
title: MediaWiki 转储
---
>[MediaWiki XML 转储文件](https://www.mediawiki.org/wiki/Manual:Importing_XML_dumps) 包含维基的内容（包含所有修订版本的维基页面），但不包含站点相关的数据。XML 转储文件不会创建维基数据库的完整备份，该文件不包含用户账户、图片、编辑日志等。

本文介绍如何将 MediaWiki XML 转储文件加载成我们可以用于下游处理的文档格式。
...
# mediawiki-utilities mwxml 有一个 bug，修复 PR 待定
...
loader = MWDumpLoader(
    ...
    encoding="utf8",
    # namespaces = [0,2,3] 可选列表，仅加载特定的命名空间。默认加载所有命名空间。
