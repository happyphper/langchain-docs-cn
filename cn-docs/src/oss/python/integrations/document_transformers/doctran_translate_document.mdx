...
title: Doctran 语言翻译
---
通过嵌入（embeddings）比较文档具有跨语言工作的优势。"Harrison says hello" 和 "Harrison dice hola" 将在向量空间中占据相似的位置，因为它们在语义上具有相同的含义。

然而，在向量化文档之前，使用 LLM **将文档翻译成其他语言**仍然是有用的。当用户预计会以不同语言查询知识库，或者对于特定语言没有可用的最先进嵌入模型时，这一点尤其有帮助。

我们可以使用 [Doctran](https://github.com/psychic-api/doctran) 库来实现这一点，该库利用 OpenAI 的函数调用功能在不同语言之间翻译文档。
...
翻译文档后，结果将作为一个新文档返回，其 `page_content` 已翻译为目标语言。
...
翻译文档后，结果将作为一个新文档返回，其 `page_content` 已翻译为目标语言。当文档被分块成多个部分时，异步版本将提高性能。它还会确保以正确的顺序返回输出。
