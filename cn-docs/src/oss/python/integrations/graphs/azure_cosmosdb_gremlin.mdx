...
---
>[Azure Cosmos DB for Apache Gremlin](https://learn.microsoft.com/en-us/azure/cosmos-db/gremlin/introduction) 是一项图数据库服务，可用于存储包含数十亿顶点和边的大规模图。您可以以毫秒级延迟查询这些图，并轻松演进图结构。
...
本笔记本展示了如何使用 LLM 为支持 `Gremlin` 查询语言的图数据库提供自然语言查询接口。
...
对于 Gremlin，请始终为每个 Node 添加一个名为 'label' 的属性。
如果未设置 label，则使用 Node.type 作为 label。
对于 Cosmos，使用自然 ID 是有意义的，因为它们在图浏览器中可见。
...
```python
# 底层的 python-gremlin 在 notebook 中运行时存在问题
# 以下行是解决该问题的变通方法
nest_asyncio.apply()
