...
title: AskNews
---
> [AskNews](https://asknews.app) 通过单一自然语言查询，为任何 LLM 注入最新的全球新闻（或历史新闻）。具体来说，AskNews 每天通过翻译、总结、提取实体并将其索引到热和冷向量数据库中来丰富超过 30 万篇文章。AskNews 将这些向量数据库放在一个低延迟的端点上供您使用。当您查询 AskNews 时，您会收到一个经过提示优化的字符串，其中包含所有最相关的丰富信息（例如实体、分类、翻译、摘要）。这意味着您无需管理自己的新闻 RAG，也无需担心如何以简洁的方式正确地向您的 LLM 传达新闻信息。
> AskNews 也致力于透明度，这就是为什么我们的覆盖范围受到监控，并跨越数百个国家、13 种语言和 5 万个来源实现多样化。如果您想跟踪我们的来源覆盖情况，可以访问我们的[透明度仪表板](https://asknews.app/en/transparency)。
...
retriever = AskNewsRetriever(
    ...
    method="kw",  # 默认为 "nl"，自然语言，也可以是 "kw" 用于关键词搜索
...
```text
"根据第二篇新闻文章提供的信息，美联储政策对科技行业的影响是负面的。文章提到，由于预期利率将在更长时间内保持高位，美国股市经历了显著下跌。利率上升尤其影响了科技行业，英特尔和微软等公司股价下跌。高利率可能导致企业借贷成本增加，从而抑制投资和支出。在科技行业，公司通常依赖借贷来资助研发和其他增长计划，更高的利率可能尤其具有挑战性。\n\n因此，美联储维持高利率的政策对科技股产生了不利影响，导致其市场估值下降。"
```
