...
实现 `RAG` 需要组织执行多个繁琐的步骤，将数据转换为嵌入（向量），将嵌入存储在专门的向量数据库中，并构建与数据库的自定义集成以搜索和检索与用户查询相关的文本。这可能非常耗时且效率低下。

借助 `Amazon Bedrock 知识库`，您只需指向 `Amazon S3` 中数据的位置，`Amazon Bedrock 知识库` 就会处理整个到向量数据库的摄取工作流。如果您没有现有的向量数据库，Amazon Bedrock 会为您创建一个 Amazon OpenSearch Serverless 向量存储。对于检索，可以通过 Retrieve API 使用 LangChain - Amazon Bedrock 集成，从知识库中检索与用户查询相关的结果。
