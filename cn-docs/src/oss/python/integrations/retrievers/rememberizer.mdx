...
本笔记本展示了如何从 `Rememberizer` 检索文档，并将其转换为下游使用的 Document 格式。

# 准备工作

你需要一个 API 密钥：你可以在 [https://rememberizer.ai](https://rememberizer.ai/) 创建通用知识库后获取一个。一旦你有了 API 密钥，必须将其设置为环境变量 `REMEMBERIZER_API_KEY`，或者在初始化 `RememberizerRetriever` 时作为 `rememberizer_api_key` 参数传入。

`RememberizerRetriever` 具有以下参数：

- 可选参数 `top_k_results`：默认值=10。用于限制返回的文档数量。
- 可选参数 `rememberizer_api_key`：如果你没有设置环境变量 `REMEMBERIZER_API_KEY`，则必须提供此参数。

`get_relevant_documents()` 方法有一个参数 `query`：用于在 `Rememberizer.ai` 的通用知识库中查找文档的自由文本。
