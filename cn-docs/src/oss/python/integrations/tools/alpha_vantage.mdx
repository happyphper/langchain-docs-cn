...
请[在其网站上](https://www.alphavantage.co/support/#api-key)生成 `ALPHAVANTAGE_API_KEY`。
...
`_get_time_series_daily` 方法返回指定全球股票的日期、日开盘价、日最高价、日最低价、日收盘价和日成交量，涵盖最新的 100 个数据点。

```python
alpha_vantage._get_time_series_daily("IBM")
```

`_get_time_series_weekly` 方法返回指定全球股票的周最后交易日、周开盘价、周最高价、周最低价、周收盘价和周成交量，涵盖 20 多年的历史数据。

```python
alpha_vantage._get_time_series_weekly("IBM")
```

`_get_quote_endpoint` 方法是时间序列 API 的一个轻量级替代方案，返回指定股票代码的最新价格和成交量信息。
...
`search_symbol` 方法根据输入的文本返回一个股票代码列表和匹配的公司信息。
...
`_get_top_gainers_losers` 方法返回美国市场中涨幅最大的 20 只股票、跌幅最大的 20 只股票以及最活跃的股票。

```python
alpha_vantage._get_top_gainers_losers()
```

该包装器的 `run` 方法接受以下参数：from_currency, to_currency。
