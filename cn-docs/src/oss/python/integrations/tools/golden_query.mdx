...
title: Golden Query
---
>[Golden](https://golden.com) 提供了一套自然语言 API，用于通过 Golden 知识图谱进行查询和知识增强。例如，诸如 `Products from OpenAI`、`Generative ai companies with series a funding` 和 `rappers who invest` 等查询可用于检索相关实体的结构化数据。

`golden-query` langchain 工具是 [Golden Query API](https://docs.golden.com/reference/query-api) 的一个封装，它使得可以通过编程方式访问这些结果。
更多信息请参阅 [Golden Query API 文档](https://docs.golden.com/reference/query-api)。

本笔记本将介绍如何使用 `golden-query` 工具。

- 访问 [Golden API 文档](https://docs.golden.com/) 以了解 Golden API 的概览。
