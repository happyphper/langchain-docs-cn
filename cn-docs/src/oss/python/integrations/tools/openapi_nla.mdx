...
title: 自然语言 API 工具包
---
`Natural Language API` 工具包 (`NLAToolkits`) 允许 LangChain 智能体高效地规划和组合跨端点的调用。
...
```python
# 选择要使用的 LLM。这里，我们使用 gpt-3.5-turbo-instruct
llm = OpenAI(
    temperature=0, max_tokens=700, model_name="gpt-3.5-turbo-instruct"
)  # 你可以在这里切换不同的核心 LLM。
```

### 接下来，加载 Natural Language API 工具包
...
行动输入：指示 AI 行动代表的内容。
观察：智能体的响应
...（此 思考/行动/行动输入/观察 可以重复 N 次）
思考：我现在知道最终答案了。用户看不到我的任何观察、API 响应、链接或工具。
最终答案：对原始输入问题的最终答案，包含适量的细节

当你用最终答案回应时，请记住，你回应的对象无法看到你的任何 思考/行动/行动输入/观察，因此如果那里有任何相关信息，你需要在回应中明确包含它。"""
```
...
某些端点可能需要通过访问令牌等方式进行用户身份验证。这里我们展示如何通过 `Requests` 包装器对象传递身份验证信息。

由于每个 NLATool 都为其包装的 API 提供了一个简洁的自然语言接口，顶层的对话智能体可以更轻松地整合每个端点来满足用户的请求。

**添加 Spoonacular 端点。**

1.  前往 [Spoonacular API 控制台](https://spoonacular.com/food-api/console#Profile) 并创建一个免费账户。
2.  点击 `Profile` 并在下方复制你的 API 密钥。
...
spoonacular_toolkit = NLAToolkit.from_llm_and_url(
    ...
    requests=requests,
    max_text_length=1800,  # 如果你想截断响应文本
)
```
