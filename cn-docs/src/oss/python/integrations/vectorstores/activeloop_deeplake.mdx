...
---
>[Activeloop Deep Lake](https://docs.deeplake.ai/) 作为一个多模态向量存储，可以存储嵌入向量及其元数据，包括文本、JSON、图像、音频、视频等。它可以将数据保存在本地、您的云端或 Activeloop 存储中。它执行混合搜索，包括嵌入向量及其属性。
...
设置 `read_only=True` 可以在不需要更新时防止意外修改向量存储。这确保了数据除非明确意图，否则保持不变。通常，指定此参数以避免意外更新是一个良好的实践。
...
默认情况下，Deep Lake 数据集存储在本地。要将它们存储在内存中、Deep Lake 托管数据库中或任何对象存储中，您可以在创建向量存储时[提供相应的路径和凭据](https://docs.deeplake.ai/latest/getting-started/storage-and-creds/storage-options/)。某些路径需要在 Activeloop 注册并创建 API 令牌，该令牌可以[在此处获取](https://app.activeloop.ai/)
...
此外，在 similarity_search 方法中支持执行查询，其中可以使用 Deep Lake 的张量查询语言（TQL）来指定查询。
