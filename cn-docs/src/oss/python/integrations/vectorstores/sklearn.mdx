...
title: scikit-learn
---
>[scikit-learn](https://scikit-learn.org/stable/) 是一个开源的机器学习算法集合，其中包含 [k 最近邻](https://scikit-learn.org/stable/modules/generated/sklearn.neighbors.NearestNeighbors.html) 的一些实现。`SKLearnVectorStore` 封装了此实现，并增加了将向量存储持久化为 json、bson（二进制 json）或 Apache Parquet 格式的功能。
...
# # 如果你计划使用 bson 序列化，请同时安装：
pip install -qU  bson

# # 如果你计划使用 parquet 序列化，请同时安装：
pip install -qU  pandas pyarrow
```

要使用 OpenAI 嵌入，你需要一个 OpenAI 密钥。你可以在 [platform.openai.com/account/api-keys](https://platform.openai.com/account/api-keys) 获取，也可以随意使用任何其他嵌入。
