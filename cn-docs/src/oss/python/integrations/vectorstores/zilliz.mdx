...
要运行，您需要启动并运行一个 `Zilliz Cloud` 实例。以下是[安装说明](https://zilliz.com/cloud)。
...
ZILLIZ_CLOUD_API_KEY = ""  # 示例: "*********" (用于无服务器集群，可作为用户名和密码的替代)
```
...
vector_db = Milvus.from_documents(
    ...
    connection_args={
        ...
        # "token": ZILLIZ_CLOUD_API_KEY,  # API 密钥，用于无服务器集群，可作为用户名和密码的替代
        "secure": True,
    },
)
```
