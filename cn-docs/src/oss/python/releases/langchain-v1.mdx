...
在底层，@[`create_agent`] 构建于基本的智能体循环之上——调用模型，让它选择要执行的工具，然后在它不再调用工具时结束：
...
中间件是 @[`create_agent`] 的定义性功能。它提供了一个高度可定制的入口点，提升了你能构建的功能上限。
...
因为 @[`create_agent`] 构建于 [LangGraph](/oss/langgraph) 之上，你自动获得对长期运行和可靠智能体的内置支持，包括：
...
新的 @[`content_blocks`][BaseMessage(content_blocks)] 属性引入了一种适用于跨供应商的消息内容标准表示：
...
请参阅我们的 [迁移指南](/oss/migrate/langchain-v1) 以获取帮助，将你的代码更新到 LangChain v1。
