...
<strong>这是直接部署智能体服务器（无需 LangSmith 平台）的设置页面。</strong>
...
`1` 和 `2` 是同一实例中的不同数据库编号，但 `<hostname_1>` 是共享的。<strong>不能将相同的数据库编号用于不同的部署</strong>。
...
`<database_name_1>` 和 `database_name_2` 是同一实例中的不同数据库，但 `<hostname_1>` 是共享的。<strong>不能将相同的数据库用于不同的部署</strong>。
