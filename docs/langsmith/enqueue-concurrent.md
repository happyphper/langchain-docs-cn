...
---
本指南假设您已了解什么是双文本处理（double-texting），您可以在[双文本处理概念指南](/langsmith/double-texting)中了解更多。

本指南将介绍双文本处理的 `enqueue` 选项，该选项将中断任务加入队列，并按客户端接收的顺序执行。以下是一个使用 `enqueue` 选项的简单示例。

在 [Agent Server](/langsmith/agent-server) 中创建运行时，`enqueue` 是默认的双文本处理（多任务）策略。
...
然后，导入所需的包并实例化客户端、助手（assistant）和线程（thread）。
...
现在，让我们启动两个运行（run），第二个运行使用 "enqueue" 多任务策略来中断第一个运行：
