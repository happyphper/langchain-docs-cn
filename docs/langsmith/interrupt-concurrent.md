...
本指南将介绍双文本的 `interrupt` 选项，该选项会中断图（graph）的先前运行，并使用双文本启动一个新的运行。此选项不会删除第一次运行，而是将其保留在数据库中，但将其状态设置为 `interrupted`。以下是使用 `interrupt` 选项的快速示例。
...
现在，让我们导入所需的包并实例化我们的客户端、助手（assistant）和线程（thread）。
...
我们可以看到线程中包含了第一次运行的部分数据以及第二次运行的数据：
