...
---
LangSmith Fetch 是一个命令行界面（CLI）工具，用于从您的 LangSmith 项目中检索追踪数据（[运行记录](/langsmith/observability-concepts#runs)、[追踪记录](/langsmith/observability-concepts#traces) 和 [会话线程](/langsmith/observability-concepts#threads)）。它允许您直接在终端和开发工作流中使用 LangSmith 的追踪和调试功能。

您可以将 LangSmith Fetch 用于以下场景：

- **即时调试**：通过单个命令获取最近一次失败或意外的智能体运行的追踪记录。
- **批量导出分析**：将大量追踪记录或整个会话线程导出为 JSON 文件，用于离线分析、构建[评估](/langsmith/evaluation-concepts)数据集或进行[回归测试](/langsmith/evaluation-types#regression-tests)。
- **基于终端的工作流**：将追踪数据集成到您现有的工具中；例如，将输出通过管道传递给 `jq` 等 Unix 工具，或将追踪记录输入到 AI 编程助手进行自动分析。
...
CLI 将自动获取 `LANGSMITH_PROJECT` 中的追踪记录或会话线程。请将 `your-project-name` 替换为您的 LangSmith 项目名称（如果项目不存在，将在首次使用时自动创建）。

要查找您的项目 UUID，请参阅 [查找项目和追踪 ID](#find-project-and-trace-ids)。

当您设置 `LANGSMITH_PROJECT` 环境变量时，`langsmith-fetch` 会自动查找项目 UUID 并将两者保存到 `~/.langsmith-cli/config.yaml`。默认情况下，您无需在 `langsmith-fetch` 命令中包含项目 UUID。但是，您可以通过 `--project-uuid` CLI 标志[指定不同的项目 UUID](#override-the-configured-tracing-project) 来覆盖已配置的项目。

<Note>

要获取会话线程，`langsmith-fetch` <strong>必须</strong> 有项目 UUID。因此，请确保设置了 `LANGSMITH_PROJECT` 环境变量，或者在运行 [`langsmith-fetch thread <thread-id>`](#fetch-a-trace-or-thread) 时手动指定 `--project-uuid <project-id>`。
...
```
使用 langsmith-fetch 分析我 LangSmith 项目中最近的 3 个会话线程，寻找潜在的改进点
```

许多助手会使用 `langsmith-fetch --help` 命令来了解如何使用 CLI 并完成您的请求。

## 查找项目和追踪 ID

在大多数情况下，您不需要手动查找 ID（CLI 默认使用您[环境中的项目名称](#setup)和最新的追踪记录）。但是，如果您想通过 ID 获取特定项目，可以在 [LangSmith UI](https://smith.langchain.com) 中找到这些 ID：

- **项目 UUID**：每个项目都有一个唯一的 ID（UUID）。您可以在项目的 URL 中找到它，或者将鼠标悬停在项目名称旁边的 <Icon icon="link-simple"/> **ID** 上。此 UUID 可与 CLI 命令的 `--project-uuid` 标志一起使用。
- **追踪 ID**：每个追踪记录（单次执行）都有一个 ID。在 **运行记录** 视图中，点击特定的运行记录即可查看其追踪 ID（可从追踪详情面板复制）。如果您有 ID，可以使用 `langsmith-fetch trace <trace-id>` 来检索该确切的追踪记录。
...
| `traces [directory]`  | 从项目中获取最近的追踪记录（多个）  | 将每个追踪记录保存为给定目录中的 JSON 文件，如果未提供目录则打印到标准输出。**提示：** 对于[批量导出](#fetch-multiple)，建议使用目录。 |
...
- `json`：格式良好且语法高亮的 JSON 输出。如果您想检查原始数据结构或将其通过管道传递给 JSON 处理工具，请使用此格式。

```bash
langsmith-fetch trace <trace-id> --format json
```

- `raw`：紧凑的 JSON，没有额外的空白字符。这对于将输出通过管道传递给其他程序（例如，使用 `jq` 或直接保存）而不需要额外格式化非常有用。
...
要获取会话线程，`langsmith-fetch thread` 也需要项目 UUID。请确保已配置 [`LANGSMITH_PROJECT` 环境变量](#setup)，或者使用 `--project-uuid` 标志来定义会话线程所属的项目：

```bash
langsmith-fetch thread <thread-id> --project-uuid <project-id>
```

![以默认 pretty 格式获取单个追踪记录的输出](/langsmith/images/langsmith-fetch-output-single.png)
...
如果您省略目录和 `--limit`，工具将把最近的一条追踪记录的结果输出到您的终端。
...
- 自定义模式：使用带有占位符的 `--filename-pattern`：
  ...
  - 支持格式说明符：例如 `{index:03d}` 用于零填充的数字。

### 包含元数据和反馈

您可以包含与追踪记录关联的[运行元数据](/langsmith/observability-concepts#metadata)和任何[反馈](/langsmith/observability-concepts#feedback)：
...
运行此命令只会从该项目获取追踪记录，它不会修改 `~/.langsmith-cli/config.yaml` 中已配置的 [LangSmith 项目](#setup)。
...
此命令检索自 2025 年 12 月 1 日以来发生的所有会话线程，将每个对话保存为 `./my_threads` 下的 JSON 文件。这对于导出聊天记录或构建多轮对话的回归测试非常有用。您也可以对会话线程使用 `--limit` 来获取特定数量的最近会话线程，`--last-n-minutes` 在此处同样适用。
