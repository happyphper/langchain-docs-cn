---
title: 如何在同一个线程上运行多个智能体
sidebarTitle: Run multiple agents on the same thread
---
在 LangSmith 部署中，线程（thread）并不显式地与特定的智能体（agent）关联。
这意味着您可以在同一个线程上运行多个智能体，从而允许不同的智能体从初始智能体的进度继续执行。

在本示例中，我们将创建两个智能体，然后在同一个线程上调用它们。
您将看到第二个智能体会使用第一个智能体在线程中生成的[检查点](/oss/langgraph/graph-api#checkpointer-state)信息作为上下文来响应。
...
现在，我们可以在默认智能体上运行它，并看到第二个智能体知道初始问题，并且能够回答“那你呢？”这个问题：
