...
此任务使用可能长时间运行的 <strong>变更操作</strong> (`ALTER TABLE DELETE`)，这是开销较大的操作，可能会影响 ClickHouse 的性能。我们建议仅在非高峰时段（夜间和周末）运行这些操作。在测试中，使用 <strong>1 个并发活动</strong> 变更（默认值）时，我们没有观察到显著的 CPU、内存或延迟增加。
