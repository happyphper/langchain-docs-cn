...
<strong>订阅</strong>：我们的更新日志包含一个 [RSS 源](https://docs.langchain.com/langsmith/self-hosted-changelog/rss.xml)，可以集成到 [Slack](https://slack.com/help/articles/218688467-Add-RSS-feeds-to-Slack)、[电子邮件](https://zapier.com/apps/email/integrations/rss/1441/send-new-rss-feed-entries-via-email)、Discord 机器人（如 [Readybot](https://readybot.io/) 或 [RSS Feeds to Discord Bot](https://rss.app/en/bots/rssfeeds-discord-bot)）以及其他订阅工具中。

</Callout>

[自托管 LangSmith](/langsmith/self-hosted) 是企业版计划的一个附加组件，专为我们规模最大、对安全性要求最高的客户设计。更多详情，请参阅[定价](https://www.langchain.com/pricing)。如果您希望获取许可证密钥以在您的环境中试用 LangSmith，请[联系我们的销售团队](https://www.langchain.com/contact-sales)。
...
- 允许在 SSO 的 JIT 配置（仅限客户端密钥模式的 OAuth）的同时启用邀请功能。
- 为管理操作添加了自助式审计日志（私有预览版）。
...
- 为 Playground 添加了 GPT-5.2 模型支持。
- 添加了对执行器 Pod 设置内存限制的支持。
...
- 修复了使用子路径时 API 文档按钮未重定向到正确 URL 的问题。
- 性能改进和错误修复。
...
- 修复了在自托管部署中禁用时，Agent Builder 引导和侧边导航的可见性问题。
...
- 为所有 SAQ（Simple Async Queue）队列添加了出队超时设置，以提高可靠性。
- 性能改进和错误修复。
...
- 为 Playground 添加了 Claude Opus 4.5 模型支持。
- 更新了数据平面操作员版本。
- 在配置了 basePath 时添加了 `LANGCHAIN_ENDPOINT` 环境变量。
...
- 将 LangSmith 和数据平面 chart 中的操作员版本更新至 0.1.21。
...
更多 Helm chart 版本可在 [`langchain-ai/helm` GitHub 仓库](https://github.com/langchain-ai/helm/releases)中找到。
