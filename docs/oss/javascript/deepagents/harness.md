...
[`FilesystemMiddleware`](/oss/javascript/deepagents/middleware) 会在工具调用结果超过令牌阈值时，自动将其逐出到文件系统，从而防止上下文窗口饱和。
...
**配置：**
- 在其[模型配置文件](/oss/javascript/langchain/models#model-profiles)中，当达到模型 `max_input_tokens` 的 85% 时触发
- 保留 10% 的令牌作为近期上下文
