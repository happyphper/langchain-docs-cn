...
title: Arcjet Redact
---
[Arcjet](https://arcjet.com) 的 redact 集成允许您在将提示发送给聊天模型之前，从中移除敏感的用户信息。

Arcjet Redact 完全在您自己的机器上运行，从不将数据发送到任何其他地方，确保了顶级的隐私和性能。

Arcjet Redact 对象本身并不是一个聊天模型，而是包装了一个 LLM。它会移除输入文本中的敏感信息，然后在返回结果之前，将包装的聊天模型的输出中的信息恢复。
...
const arcjetRedactOptions = {
  ...
  // 您可以提供一个自定义的检测函数来检测我们尚不支持的实体。
  // 它接收一个 token 列表，您需要返回一个已识别类型的列表或 undefined。
  // 如果您使用了自定义类型，应将返回的 undefined 类型添加到 entities 列表中。
