...
本文将帮助您开始使用 Cohere 的[聊天模型](/oss/javascript/langchain/models)。有关 `ChatCohere` 所有功能和配置的详细文档，请参阅 [API 参考](https://api.js.langchain.com/classes/langchain_cohere.ChatCohere.html)。
...
如果您希望自动追踪模型调用，还可以通过取消注释以下代码来设置您的 [LangSmith](https://docs.langchain.com/langsmith/home) API 密钥：
...
该 API 还允许使用非静态文档的其他连接。
例如，他们的 `web-search` 连接器允许您传入查询，API 将在网络上搜索相关文档。
下面的示例演示了如何使用此功能。
...
我们可以在 `additional_kwargs` 对象中看到 API 请求执行了几项操作：

- 执行了搜索查询，将结果数据存储在 `searchQueries` 和 `searchResults` 字段中。在 `searchQueries` 字段中，我们看到他们重新表述了我们的查询以获得更好的结果。
- 根据搜索查询生成了三个文档。
- 生成了一个引用列表。
- 根据上述操作和内容生成了最终响应。
...
有关 ChatCohere 所有功能和配置的详细文档，请参阅 [API 参考](https://api.js.langchain.com/classes/langchain_cohere.ChatCohere.html)。
