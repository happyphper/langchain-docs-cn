...
LangChain <a href="https://reference.langchain.com/javascript/classes/_langchain_openai.ChatOpenAI.html" target="_blank" rel="noreferrer" class="link"><code>ChatOpenAI</code></a> 集成位于 `@langchain/openai` 包中：
...
某些代理或第三方提供商提供了与 OpenAI 基本相同的 API 接口，但不支持较新添加的 `stream_options` 参数来返回流式使用量。您可以通过如下方式禁用流式使用量来使用 <a href="https://reference.langchain.com/javascript/classes/_langchain_openai.ChatOpenAI.html" target="_blank" rel="noreferrer" class="link"><code>ChatOpenAI</code></a> 访问这些提供商：
...
如果您使用的模型支持自定义工具，可以使用 <a href="https://reference.langchain.com/javascript/classes/_langchain_openai.ChatOpenAI.html" target="_blank" rel="noreferrer" class="link"><code>ChatOpenAI</code></a> 类和 `customTool` 函数来创建自定义工具。
