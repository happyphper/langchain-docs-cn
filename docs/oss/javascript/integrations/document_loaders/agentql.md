...
`AgentQLLoader` 目前一次只加载一个 <a href="https://reference.langchain.com/javascript/classes/_langchain_core.documents.Document.html" target="_blank" rel="noreferrer" class="link"><code>Document</code></a>。因此，`load()` 和 `lazy_load()` 的行为相同：
