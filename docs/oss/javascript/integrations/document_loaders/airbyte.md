...
本文档介绍如何将 Airbyte 中的任何数据源加载到 LangChain 文档中。
...
默认情况下，`AirbyteLoader` 将从数据流加载任何结构化数据，并输出 YAML 格式的文档。
...
`AirbyteLoader` 的一个强大功能是能够从上游源加载大型文档。处理大型数据集时，默认的 `.load()` 行为可能速度较慢且占用大量内存。为了避免这种情况，您可以使用 `.lazy_load()` 方法以更节省内存的方式加载文档。
...
- `source` (str, 必需)：要加载的 Airbyte 数据源名称。
- `stream` (str, 必需)：要加载的数据流名称（Airbyte 数据源可以返回多个数据流）
- `config` (dict, 必需)：Airbyte 数据源的配置
- `template` (PromptTemplate, 可选)：用于格式化文档的自定义提示模板
- `include_metadata` (bool, 可选，默认为 True)：是否将所有字段作为元数据包含在输出文档中

大部分配置将在 `config` 中，您可以在 [Airbyte 文档](https://docs.airbyte.com/integrations/) 中每个数据源的“配置字段参考”部分找到具体的配置选项。
