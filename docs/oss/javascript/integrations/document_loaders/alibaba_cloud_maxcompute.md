...
---
>[阿里云 MaxCompute](https://www.alibabacloud.com/product/maxcompute)（原名 ODPS）是一个通用、完全托管、多租户的大规模数据仓库数据处理平台。MaxCompute 支持多种数据导入解决方案和分布式计算模型，使用户能够高效查询海量数据集、降低生产成本并确保数据安全。

`MaxComputeLoader` 允许您执行 MaxCompute SQL 查询，并将结果按行加载为文档，每行一个文档。
...
要实例化加载器，您需要提供要执行的 SQL 查询、MaxCompute 端点（endpoint）和项目名称，以及您的访问 ID 和秘密访问密钥。访问 ID 和秘密访问密钥可以直接通过 `access_id` 和 `secret_access_key` 参数传入，也可以设置为环境变量 `MAX_COMPUTE_ACCESS_ID` 和 `MAX_COMPUTE_SECRET_ACCESS_KEY`。
...
您可以使用 `page_content_columns` 和 `metadata_columns` 参数来配置应将哪些列的子集加载为 Document 的内容，哪些加载为元数据。
...
loader = MaxComputeLoader.from_params(
    ...
    endpoint,
    project,
    page_content_columns=["content"],  # 指定 Document 页面内容
    metadata_columns=["id", "meta_info"],  # 指定 Document 元数据
    access_id=ACCESS_ID,
    secret_access_key=SECRET_ACCESS_KEY,
)
data = loader.load()
