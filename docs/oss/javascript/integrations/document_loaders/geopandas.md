...
title: Geopandas
---
[Geopandas](https://geopandas.org/en/stable/index.html) 是一个开源项目，旨在简化 Python 中地理空间数据的处理。
...
以 [`Open City Data`](/oss/javascript/integrations/document_loaders/open_city_data) 为例，创建一个 GeoPandas 数据框作为示例输入。
...
将 GeoPandas 数据框加载为 <a href="https://reference.langchain.com/javascript/classes/_langchain_core.documents.Document.html" target="_blank" rel="noreferrer" class="link"><code>Document</code></a> 以供下游处理（嵌入、聊天等）。

默认情况下，`geometry` 列将作为 `page_content`，所有其他列则放入 `metadata` 中。
