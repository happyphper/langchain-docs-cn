...
本指南将介绍如何使用 El Carro LangChain 集成，通过 `ElCarroLoader` 和 `ElCarroDocumentSaver` 来[保存、加载和删除 LangChain 文档](/oss/javascript/integrations/document_loaders)。此集成适用于任何 Oracle 数据库，无论其运行在何处。
