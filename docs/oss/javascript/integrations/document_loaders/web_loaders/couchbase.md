...
有关使用 SQL++（用于 JSON 的 SQL）查询文档的帮助，请查阅 [文档](https://docs.couchbase.com/server/current/n1ql/n1ql-language-reference/index.html)。
...
您可以通过调用加载器的 `load` 方法来获取文档。它将返回包含所有文档的列表。如果您想避免此阻塞调用，可以调用返回迭代器的 `lazy_load` 方法。
...
可以使用 `pageContentFields` 参数指定构成文档内容部分的字段。
