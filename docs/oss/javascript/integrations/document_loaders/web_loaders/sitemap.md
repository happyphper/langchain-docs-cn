...
title: SitemapLoader
---
本笔记本将介绍如何使用 [`SitemapLoader`](https://api.js.langchain.com/classes/_langchain_community.document_loaders_web_sitemap.SitemapLoader.html) 类将站点地图加载到 `Document` 中。
...
传入的 URL 必须包含指向站点地图的 `.xml` 路径，否则将在 URL 后附加默认的 `/sitemap.xml`。
