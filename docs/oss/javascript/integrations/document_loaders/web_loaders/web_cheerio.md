...
title: Cheerio
---
本笔记本提供了快速入门 [CheerioWebBaseLoader](/oss/javascript/integrations/document_loaders/) 的概述。有关 CheerioWebBaseLoader 所有功能和配置的详细文档，请前往 [API 参考](https://api.js.langchain.com/classes/langchain_community_document_loaders_web_cheerio.CheerioWebBaseLoader.html)。
...
然而，Cheerio 不模拟 Web 浏览器，因此无法在页面上执行 JavaScript 代码。这意味着它无法从需要 JavaScript 渲染的动态网页中提取数据。为此，您可以使用 [`PlaywrightWebBaseLoader`](/oss/javascript/integrations/document_loaders/web_loaders/web_playwright) 或 [`PuppeteerWebBaseLoader`](/oss/javascript/integrations/document_loaders/web_loaders/web_puppeteer)。
...
如果您希望自动跟踪模型调用，也可以通过取消注释以下内容来设置您的 [LangSmith](https://docs.langchain.com/langsmith/home) API 密钥：
...
`CheerioWebBaseLoader` 在实例化加载器时支持额外的配置。以下是一个示例，展示了如何使用传递的 `selector` 字段，使其仅从提供的 HTML 类名加载内容：
...
有关 CheerioWebBaseLoader 所有功能和配置的详细文档，请前往 [API 参考](https://api.js.langchain.com/classes/langchain_community_document_loaders_web_cheerio.CheerioWebBaseLoader.html)。
