...
---
在摄取 HTML 文档以供后续检索时，我们通常只对网页的实际内容感兴趣，而不是其语义。
使用 MozillaReadabilityTransformer 剥离 HTML 标签可以得到内容更丰富的文本块，从而提高检索效果。
...
```
虽然转换器本身不需要，但下面的使用示例需要 [`cheerio`](https://www.npmjs.com/package/cheerio) 来进行网页抓取：
...
下面的示例抓取了一个 Hacker News 帖子，根据 HTML 标签进行分割以基于标签的语义信息对文本块进行分组，然后从各个块中提取内容：
...
你可以向转换器传递任何 [`@mozilla/readability` 包接受的参数](https://www.npmjs.com/package/@mozilla/readability) 来自定义其工作方式。
