...
Hugging Face 的 rerank 模型可以通过 `OpenVINOReranker` 类得到 OpenVINO 的支持。如果你有 Intel GPU，可以指定 `model_kwargs={"device": "GPU"}` 来在其上运行推理。
...
现在，让我们使用 `OpenVINOReranker` 作为压缩器，用 `ContextualCompressionRetriever` 包装我们的基础检索器。
...
你可以使用 `OVModelForSequenceClassification` 将你的重排序模型导出为 OpenVINO IR 格式，并从本地文件夹加载模型。
