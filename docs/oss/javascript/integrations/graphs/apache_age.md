...
title: Apache AGE
---
>[Apache AGE](https://age.apache.org/) 是一个 PostgreSQL 扩展，提供图数据库功能。AGE 是 A Graph Extension 的缩写，其灵感来源于 Bitnine 基于 PostgreSQL 10 的分支 AgensGraph，后者是一个多模型数据库。该项目的目标是创建一个能够同时处理关系模型和图模型数据的单一存储，以便用户可以使用标准的 ANSI SQL 以及图查询语言 openCypher。`Apache AGE` 存储的数据元素是节点、连接它们的边以及节点和边的属性。

> 本笔记本展示了如何使用 LLM 为支持 `Cypher` 查询语言的图数据库提供自然语言接口。

>[Cypher](https://en.wikipedia.org/wiki/Cypher_(query_language)) 是一种声明式图查询语言，允许在属性图中进行富有表现力和高效的数据查询。

## 设置

您需要有一个运行中的 `Postgre` 实例，并已安装 AGE 扩展。一个测试选项是使用官方的 AGE Docker 镜像运行一个 Docker 容器。
...
假设您的数据库是空的，您可以使用 Cypher 查询语言来填充它。以下 Cypher 语句是幂等的，这意味着无论您运行一次还是多次，数据库信息都将相同。
