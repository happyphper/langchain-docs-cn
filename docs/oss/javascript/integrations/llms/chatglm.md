...
title: ChatGLM
---
[ChatGLM-6B](https://github.com/THUDM/ChatGLM-6B) 是一个基于通用语言模型（GLM）框架的开源双语语言模型，拥有 62 亿参数。通过量化技术，用户可以在消费级显卡上本地部署（在 INT4 量化级别下仅需 6GB GPU 显存）。

[ChatGLM2-6B](https://github.com/THUDM/ChatGLM2-6B) 是开源双语（中英）对话模型 ChatGLM-6B 的第二代版本。它保留了一代模型流畅的对话体验和低部署门槛的特点，同时引入了性能更强、上下文更长、推理更高效等新特性。

[ChatGLM3](https://github.com/THUDM/ChatGLM3) 是智谱 AI 与清华大学 KEG 实验室联合发布的新一代预训练对话模型。ChatGLM3-6B 是 ChatGLM3 系列中的开源模型。
...
以下示例展示了如何使用 LangChain 与 ChatGLM2-6B 推理接口进行文本补全交互。
ChatGLM-6B 和 ChatGLM2-6B 具有相同的 API 规范，因此本示例对两者均适用。
...
# 仅当你希望 LLM 对象跟踪对话历史并将累积的上下文发送到后端模型 API（使其具有状态）时，才开启 with_history。默认情况下它是无状态的。
