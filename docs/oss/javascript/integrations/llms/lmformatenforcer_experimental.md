...
它的工作原理是将字符级解析器与分词器前缀树相结合，只允许包含可能形成有效格式的字符序列的令牌。
...
请注意，Llama2 [需要申请批准才能访问模型](https://huggingface.co/meta-llama/Llama-2-7b-chat-hf)。
...
首先，让我们通过检查没有结构化解码时模型的输出来建立一个定性基线。
...
让我们再试一次，这次向模型提供 Action 输入的 JSON 模式。
...
这意味着，如果你需要为 API 调用或类似场景格式化 JSON，只要你能生成模式（从 pydantic 模型或通用模式），你就可以使用这个库来确保 JSON 输出是正确的，幻觉风险最小。
...
LMFormatEnforcer 还有另一种模式，使用正则表达式来过滤输出。请注意，它在底层使用了 [interegular](https://pypi.org/project/interegular/)，因此不支持 100% 的正则表达式功能。
