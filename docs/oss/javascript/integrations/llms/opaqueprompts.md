...
title: OpaquePrompts
---
[OpaquePrompts](https://opaqueprompts.readthedocs.io/en/latest/) 是一项服务，它使应用程序能够利用语言模型的能力，同时不损害用户隐私。OpaquePrompts 专为可组合性和易于集成到现有应用程序与服务中而设计，可通过简单的 Python 库以及 LangChain 来使用。或许更重要的是，OpaquePrompts 利用了[机密计算](https://en.wikipedia.org/wiki/Confidential_computing)的能力，以确保即使是 OpaquePrompts 服务本身也无法访问其正在保护的数据。
...
访问 OpaquePrompts API 需要一个 API 密钥，您可以通过在 [OpaquePrompts 网站](https://opaqueprompts.opaque.co/) 上创建账户来获取。拥有账户后，您可以在 [API 密钥页面](https:opaqueprompts.opaque.co/api-keys) 找到您的 API 密钥。
...
将 OpaquePrompts 应用到您的应用程序可以非常简单，只需使用 OpaquePrompts 类包装您的 LLM，将 `llm=OpenAI()` 替换为 `llm=OpaquePrompts(base_llm=OpenAI())`。
...
OpaquePrompts 将自动检测敏感数据并用占位符替换它。
...
响应通过将占位符替换为原始敏感数据来进行去敏化处理。

```
# 来自 OpaquePrompts 的去敏化 LLM 响应
...
如果直接替换无法提供您所需的灵活性，也有一些函数可以与 LangChain 表达式一起使用。
