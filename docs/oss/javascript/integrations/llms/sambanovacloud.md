...
<strong>您当前正在查看的是关于将 SambaNovaCloud 模型用作文本补全模型的文档页面。我们建议您使用 [聊天补全模型](/oss/javascript/langchain/models)。</strong>

您可能正在寻找 [SambaNova 聊天模型](/oss/javascript/integrations/chat/sambanova/)。

</Warning>

**[SambaNova](https://sambanova.ai/) 的 [SambaNovaCloud](https://cloud.sambanova.ai/)** 是一个用于使用开源模型进行推理的平台。
...
有关 `SambaNovaCloud` llm 所有功能和配置的详细文档，请访问 API 参考：[python.langchain.com/api_reference/community/llms/langchain_community.llms.sambanova.SambaNovaCloud.html](https://python.langchain.com/api_reference/community/llms/langchain_community.llms.sambanova.SambaNovaCloud.html)
