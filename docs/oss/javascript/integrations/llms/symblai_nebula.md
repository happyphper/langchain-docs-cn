...
---
[Nebula](https://symbl.ai/nebula/) 是由 [Symbl.ai](https://symbl.ai) 构建的大型语言模型 (LLM)。它经过训练，能够执行基于人类对话的生成任务。Nebula 擅长对对话中的细微差别进行建模，并在对话上执行任务。

Nebula 文档：[docs.symbl.ai/docs/nebula-llm](https://docs.symbl.ai/docs/nebula-llm)

本示例将介绍如何使用 LangChain 与 [Nebula 平台](https://docs.symbl.ai/docs/nebula-llm) 进行交互。

请确保您拥有 API 密钥。如果您还没有，请 [申请一个](https://info.symbl.ai/Nebula_Private_Beta.html)。
