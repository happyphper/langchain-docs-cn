...
>[Nebius AI Studio](https://studio.nebius.ai/) 为各种用例提供了访问一系列先进的大型语言模型和嵌入模型的 API。
...
要使用 Nebius AI Studio，你需要一个 API 密钥，可以从 [Nebius AI Studio](https://studio.nebius.ai/) 获取。API 密钥可以作为初始化参数 `api_key` 传递，或设置为环境变量 `NEBIUS_API_KEY`。
...
支持的完整模型列表可以在 [Nebius AI Studio 文档](https://studio.nebius.com/) 中找到。
...
查看 [使用示例](/oss/javascript/integrations/chat/nebius)。
...
查看 [使用示例](/oss/javascript/integrations/text_embedding/nebius)。
...
查看 [使用示例](/oss/javascript/integrations/retrievers/nebius)。
...
`NebiusRetrievalTool` 允许你基于 NebiusRetriever 为智能体（agent）创建一个工具。
