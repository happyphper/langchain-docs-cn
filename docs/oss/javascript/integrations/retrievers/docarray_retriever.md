...
title: DocArray
---
>[DocArray](https://github.com/docarray/docarray) 是一个多功能、开源的工具，用于管理您的多模态数据。它允许您按需塑造数据，并提供了使用多种文档索引后端来存储和搜索数据的灵活性。更棒的是，您可以使用您的 `DocArray` 文档索引来创建一个 `DocArrayRetriever`，并构建出色的 LangChain 应用！

本笔记本分为两个部分。[第一部分](#文档索引后端) 介绍了所有五种支持的文档索引后端。它提供了设置和索引每个后端的指导，并指导您如何构建一个用于查找相关文档的 `DocArrayRetriever`。
在[第二部分](#使用-hnswdocumentindex-进行电影检索)中，我们将选择其中一个后端，并通过一个基本示例来说明如何使用它。
...
`InMemoryExactNNIndex` 将所有文档存储在内存中。对于小型数据集来说，这是一个很好的起点，您可能不希望启动数据库服务器。

了解更多信息：[docs.docarray.org/user_guide/storing/index_in_memory/](https://docs.docarray.org/user_guide/storing/index_in_memory/)
...
了解更多信息：[docs.docarray.org/user_guide/storing/index_hnswlib/](https://docs.docarray.org/user_guide/storing/index_hnswlib/)
...
了解更多信息：[docs.docarray.org/user_guide/storing/index_weaviate/](https://docs.docarray.org/user_guide/storing/index_weaviate/)

```python
# Weaviate 后端与其他后端有一个小区别。
# 在这里，您需要用 'is_embedding=True' 来“标记”用于向量搜索的字段。
...
了解更多信息[请点击此处](https://docs.docarray.org/user_guide/storing/index_elastic/)
...
了解更多信息[请点击此处](https://docs.docarray.org/user_guide/storing/index_qdrant/)
