...
title: Elasticsearch
---
>[Elasticsearch](https://www.elastic.co/elasticsearch/) 是一个分布式的、RESTful 风格的搜索和分析引擎。它提供了一个分布式的、支持多租户的全文搜索引擎，具有 HTTP Web 接口和无模式（schema-free）的 JSON 文档。它支持关键词搜索、向量搜索、混合搜索和复杂过滤。
...
本指南将帮助你开始使用 Elasticsearch [检索器](/oss/javascript/langchain/retrieval)。有关 `ElasticsearchRetriever` 所有功能和配置的详细文档，请参阅 [API 参考](https://python.langchain.com/api_reference/elasticsearch/retrievers/langchain_elasticsearch.retrievers.ElasticsearchRetriever.html)。
...
要连接到不需要登录凭据的 Elasticsearch 实例（启动启用了安全性的 Docker 实例），请将 Elasticsearch URL、索引名称以及嵌入对象传递给构造函数。

- **本地安装 Elasticsearch**：通过在本地运行 Elasticsearch 来开始使用。最简单的方法是使用官方的 Elasticsearch Docker 镜像。更多信息请参阅 [Elasticsearch Docker 文档](https://www.elastic.co/guide/en/elasticsearch/reference/current/docker.html)。
...
此检索器位于 `langchain-elasticsearch` 包中。出于演示目的，我们还将安装 `langchain-community` 来生成文本[嵌入](/oss/javascript/integrations/text_embedding)。
...
这里我们定义到 Elasticsearch 的连接。在本示例中，我们使用本地运行的实例。或者，你可以在 [Elastic Cloud](https://cloud.elastic.co/) 中创建一个账户并开始 [免费试用](https://www.elastic.co/cloud/cloud-trial-overview)。
...
对于向量搜索，我们将仅使用随机嵌入进行说明。对于实际用例，请选择可用的 LangChain [嵌入](/oss/javascript/integrations/text_embedding) 类之一。
...
通常，用户在其 Elasticsearch 索引中已有数据时，会使用 `ElasticsearchRetriever`。这里我们索引一些示例文本文档。如果你使用 `ElasticsearchStore.from_documents` 创建了索引，那也是可以的。
...
结合向量搜索和 BM25 搜索，使用 [互逆排名融合](https://www.elastic.co/guide/en/elasticsearch/reference/current/rrf.html) (RRF) 来合并结果集。
...
请注意，查询匹配的结果位于顶部。通过过滤器的其他文档也包含在结果集中，但它们都具有相同的分数。
...
有关 `ElasticsearchRetriever` 所有功能和配置的详细文档，请参阅 [API 参考](https://python.langchain.com/api_reference/elasticsearch/retrievers/langchain_elasticsearch.retrievers.ElasticsearchRetriever.html)。
