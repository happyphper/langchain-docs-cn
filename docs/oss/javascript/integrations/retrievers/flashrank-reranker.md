...
---
>[FlashRank](https://github.com/PrithivirajDamodaran/FlashRank) 是一个超轻量级、超快速的 Python 库，用于为现有的搜索和检索流程添加重排序功能。它基于最先进的交叉编码器（cross-encoders），在此感谢所有模型所有者。
...
现在，让我们使用 `FlashrankRerank` 作为压缩器，用 `ContextualCompressionRetriever` 包装我们的基础检索器。
