...
---
>[LLMLingua](https://github.com/microsoft/LLMLingua) 利用一个紧凑、训练有素的语言模型（例如 GPT2-small、LLaMA-7B）来识别并移除提示中非必要的标记。这种方法能够实现对大型语言模型（LLM）的高效推理，在性能损失最小的情况下实现高达 20 倍的压缩。
...
现在，让我们使用 `LLMLinguaCompressor` 作为压缩器，用 `ContextualCompressionRetriever` 包装我们的基础检索器。
