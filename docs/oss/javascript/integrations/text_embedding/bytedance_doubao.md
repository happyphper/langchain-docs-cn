...
title: 字节跳动豆包嵌入模型
---
本文将帮助您开始使用 LangChain 集成 ByteDanceDoubao [嵌入模型](/oss/javascript/integrations/text_embedding)。有关 `ByteDanceDoubaoEmbeddings` 功能和配置选项的详细文档，请参阅 [API 参考](https://api.js.langchain.com/classes/_langchain_community.embeddings_bytedance_doubao.ByteDanceDoubaoEmbeddings.html)。
...
您需要注册一个 [ARK API 密钥](https://console.volcengine.com/ark/region:ark+cn-beijing/apiKey) 并将其设置为名为 `ARK_API_KEY` 的环境变量。然后，您应该为嵌入模型 [创建一个接入点](https://console.volcengine.com/ark/region:ark+cn-beijing/endpoint)，并使用该接入点的名称作为 `model`。
...
LangChain 的 ByteDanceDoubaoEmbeddings 集成位于 `@langchain/community` 包中：
...
下面，我们将使用上面初始化的 `embeddings` 对象来演示如何索引和检索数据。在此示例中，我们将使用演示用的 [`MemoryVectorStore`](/oss/javascript/integrations/vectorstores/memory) 来索引和检索一个示例文档。
...
有关 ByteDanceDoubaoEmbeddings 所有功能和配置的详细文档，请前往 [API 参考](https://api.js.langchain.com/classes/_langchain_community.embeddings_bytedance_doubao.ByteDanceDoubaoEmbeddings.html)。
