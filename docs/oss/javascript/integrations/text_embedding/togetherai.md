...
---
本文将帮助您开始使用 LangChain 的 TogetherAIEmbeddings [嵌入模型](/oss/javascript/integrations/text_embedding)。有关 `TogetherAIEmbeddings` 功能和配置选项的详细文档，请参阅 [API 参考](https://api.js.langchain.com/classes/langchain_community_embeddings_togetherai.TogetherAIEmbeddings.html)。

## 概述

### 集成详情

| 类 | 包 | 本地 | [Python 支持](https://python.langchain.com/docs/integrations/text_embedding/together/) | 下载量 | 版本 |
| :--- | :--- | :---: | :---: |  :---: | :---: |
...
要访问 TogetherAI 的嵌入模型，您需要创建一个 TogetherAI 账户，获取 API 密钥，并安装 `@langchain/community` 集成包。

### 凭证

您可以在此处[注册](https://api.together.xyz/) Together 账户并创建 API 密钥。完成后，请设置 `TOGETHER_AI_API_KEY` 环境变量：
...
LangChain 的 TogetherAIEmbeddings 集成位于 `@langchain/community` 包中：
...
嵌入模型通常用于检索增强生成 (RAG) 流程中，既用于索引数据，也用于后续检索。更详细的说明，请参阅 [**学习**](/oss/javascript/learn/) 选项卡下的 RAG 教程。
...
您可以使用 `embedDocuments` 为索引嵌入多个文本。此方法内部使用的机制可能（但不一定）与嵌入查询时不同：
