...
title: Google Books
---
支持 ReAct 模式的 Google Books 工具，允许您搜索 Google Books API。Google Books 是世界上最大的 API，以精心策划的方式跟踪书籍信息。它拥有超过 4000 万条记录，可以为用户提供海量数据。
...
您需要从 Google Books 获取一个 API 密钥。您可以访问 [https://developers.google.com/books/docs/v1/using#APIKey](https://developers.google.com/books/docs/v1/using#APIKey) 并按照步骤操作。

然后，您需要将环境变量 `GOOGLE_BOOKS_API_KEY` 设置为您的 Google Books API 密钥。

## 实例化

要实例化该工具，请导入 Google Books 工具并设置您的凭证。
...
Google Books API 可以在这里找到：[https://developers.google.com/books](https://developers.google.com/books)
