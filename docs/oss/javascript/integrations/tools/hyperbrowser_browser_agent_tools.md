...
---
[Hyperbrowser](https://hyperbrowser.ai) 是一个用于运行、管理浏览器代理以及扩展无头浏览器规模的平台。它允许您大规模启动和管理浏览器会话，并为任何网络抓取需求（例如抓取单个页面或爬取整个网站）提供易于使用的解决方案。
...
- **Claude 计算机使用工具**：与 Anthropic 的 Claude 计算机使用功能集成
...
要访问 Hyperbrowser 工具，您需要安装 `langchain-hyperbrowser` 集成包，并创建一个 Hyperbrowser 账户以获取 API 密钥。
...
`HyperbrowserOpenAICUATool` 是一个专用工具，通过 Hyperbrowser 利用 OpenAI 的计算机使用代理 (CUA) 功能。
...
Claude 计算机使用、OpenAI CUA 和浏览器使用工具提供以下可用参数：
...
- [浏览器使用 API 参考](https://docs.hyperbrowser.ai/reference/api-reference/agents/browser-use)
- [OpenAI CUA API 参考](https://docs.hyperbrowser.ai/reference/api-reference/agents/openai-cua)
- [Claude 计算机使用 API 参考](https://docs.hyperbrowser.ai/reference/api-reference/agents/claude-computer-use)
