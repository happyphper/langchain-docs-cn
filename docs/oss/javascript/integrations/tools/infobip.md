...
title: Infobip
---
本笔记本展示了如何使用 [Infobip](https://www.infobip.com/) API 包装器来发送短信和电子邮件。

Infobip 提供多种服务，但本笔记本将重点介绍短信和电子邮件服务。你可以在[这里](https://www.infobip.com/docs/api)找到关于 API 和其他渠道的更多信息。

## 设置

要使用此工具，你需要拥有一个 Infobip 账户。你可以创建一个[免费试用账户](https://www.infobip.com/docs/essentials/free-trial)。

`InfobipAPIWrapper` 使用以下命名参数，你可以在其中提供凭据：

- `infobip_api_key` - 你可以在[开发者工具](https://portal.infobip.com/dev/api-keys)中找到的 [API 密钥](https://www.infobip.com/docs/essentials/api-authentication#api-key-header)。
- `infobip_base_url` - Infobip API 的[基础 URL](https://www.infobip.com/docs/essentials/base-url)。你可以使用默认值 `https://api.infobip.com/`。

你也可以将 `infobip_api_key` 和 `infobip_base_url` 作为环境变量 `INFOBIP_API_KEY` 和 `INFOBIP_BASE_URL` 提供。
