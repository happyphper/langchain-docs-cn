...
title: 工具
---
`@langchain/openai` 包提供了与 LangChain 兼容的 OpenAI 内置工具封装。这些工具可以使用 `bindTools()` 或 <a href="https://reference.langchain.com/javascript/functions/langchain.index.createAgent.html" target="_blank" rel="noreferrer" class="link"><code>createAgent</code></a> 绑定到 `ChatOpenAI`。
