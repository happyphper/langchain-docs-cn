...
---
这将帮助您开始使用 SQL 数据库 [工具包](/oss/javascript/langchain/tools#toolkits)。有关 `SQLDatabaseToolkit` 所有功能和配置的详细文档，请参阅 [API 参考](https://python.langchain.com/api_reference/community/agent_toolkits/langchain_community.agent_toolkits.sql.toolkit.SQLDatabaseToolkit.html)。

`SQLDatabaseToolkit` 中的工具旨在与 `SQL` 数据库交互。

一个常见的应用是让智能体能够使用关系数据库中的数据回答问题，可能以迭代的方式进行（例如，从错误中恢复）。

**⚠️ 安全注意事项 ⚠️**

构建基于 SQL 数据库的问答系统需要执行模型生成的 SQL 查询。这样做存在固有风险。请确保您的数据库连接权限始终根据您的链/智能体的需求尽可能缩小范围。这将减轻（尽管不能消除）构建模型驱动系统的风险。
...
出于演示目的，我们将访问 LangChain [Hub](https://smith.langchain.com/hub) 中的一个提示。我们还需要 `langgraph` 来演示与智能体一起使用该工具包。使用该工具包并不需要此依赖。
...
下面我们将使用 `requests` 库拉取 `.sql` 文件并创建一个内存中的 SQLite 数据库。请注意，这种方法轻量级，但具有临时性且非线程安全。如果您愿意，可以按照说明将文件本地保存为 `Chinook.db`，并通过 `db = SQLDatabase.from_uri("sqlite:///Chinook.db")` 实例化数据库。
...
遵循 [SQL 问答教程](/oss/javascript/langchain/sql-agent)，下面我们为简单的问答智能体配备工具包中的工具。首先，我们拉取一个相关的提示并用其必需的参数填充它：
...
`SQLDatabaseToolkit` 实现了一个 [.get_context](https://python.langchain.com/api_reference/community/agent_toolkits/langchain_community.agent_toolkits.sql.toolkit.SQLDatabaseToolkit.html#langchain_community.agent_t
