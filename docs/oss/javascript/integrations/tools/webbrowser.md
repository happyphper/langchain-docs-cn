...
title: 网页浏览器工具
---
Webbrowser 工具赋予你的智能体访问网站并提取信息的能力。它被描述给智能体如下：

```
当你需要在网页上查找内容或进行摘要时非常有用。输入应为一个逗号分隔的列表："包含协议的有效 URL","你想在页面上查找的内容，若为空字符串则进行摘要"。
```

它提供了两种操作模式：

- 当智能体仅传入 URL 调用时，它会生成网站内容的摘要。
- 当智能体传入 URL 和要查找内容的描述调用时，它将使用内存中的向量存储（Vector Store）来查找最相关的片段并摘要这些片段。

## 安装

要使用 Webbrowser 工具，你需要安装以下依赖：
