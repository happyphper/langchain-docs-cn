...
假设使用 @[`OpenAIEmbeddings`]，但如果您使用 Azure OpenAI，请导入 @[`AzureOpenAIEmbeddings`]。
...
创建 @[`OpenAIEmbeddings`] 和 AzureSearch 类的实例。完成此步骤后，您的 Azure AI Search 资源上应该会有一个空的搜索索引。集成模块提供了默认的架构。
