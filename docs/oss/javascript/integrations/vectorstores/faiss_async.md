...
查看 `synchronous` 版本 [请点击这里](/oss/javascript/integrations/vectorstores/faiss)。

```python
pip install -qU  faiss-gpu # 适用于支持 CUDA 7.5+ 的 GPU。
# 或者
pip install -qU  faiss-cpu # 适用于 CPU 安装
```
...
你可以通过这些函数对 FAISS 索引进行 pickle 序列化。如果你使用的嵌入模型大小为 90 MB（例如 sentence-transformers/all-MiniLM-L6-v2 或任何其他模型），生成的 pickle 文件大小将超过 90 MB，因为模型的大小也包含在总大小中。要克服这个问题，请使用下面的函数。这些函数只序列化 FAISS 索引，因此大小会小得多。如果你希望将索引存储在像 SQL 这样的数据库中，这会很有帮助。
...
FAISS 向量存储也支持过滤，但由于 FAISS 本身不支持过滤，我们必须手动进行。这是通过首先获取比 `k` 更多的结果，然后过滤它们来实现的。你可以根据元数据过滤文档。你还可以在调用任何搜索方法时设置 `fetch_k` 参数，以设置在过滤之前要获取多少文档。这里有一个小例子：
...
以下是如何在调用 `similarity_search` 时设置 `fetch_k` 参数的示例。通常你会希望 `fetch_k` 参数 >> `k` 参数。这是因为 `fetch_k` 参数是在过滤之前要获取的文档数量。如果你将 `fetch_k` 设置为一个较小的数字，你可能无法获得足够的文档进行过滤。
...
你也可以删除 id。请注意，要删除的 id 应该是 docstore 中的 id。
