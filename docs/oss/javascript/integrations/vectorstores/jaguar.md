...
4.  全主节点架构：支持并行读取和写入
5.  异常检测能力
6.  支持 RAG：将大语言模型（LLM）与专有及实时数据相结合
7.  共享元数据：支持跨多个向量索引共享元数据
8.  距离度量：欧几里得距离（Euclidean）、余弦相似度（Cosine）、内积（InnerProduct）、曼哈顿距离（Manhatten）、切比雪夫距离（Chebyshev）、汉明距离（Hamming）、杰卡德距离（Jeccard）、闵可夫斯基距离（Minkowski）
...
template = """你是一个用于问答任务的助手。使用以下检索到的上下文片段来回答问题。如果你不知道答案，就说你不知道。最多使用三句话，保持答案简洁。
问题：{question}
上下文：{context}
答案：
"""
