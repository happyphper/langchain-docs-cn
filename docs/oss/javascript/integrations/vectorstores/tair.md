...
title: Tair
---
>[Tair](https://www.alibabacloud.com/help/en/tair/latest/what-is-tair) 是 `阿里云` 开发的云原生内存数据库服务。
它提供丰富的数据模型和企业级能力，以支持您的实时在线场景，同时保持与开源 `Redis` 的完全兼容。`Tair` 还引入了基于新型非易失性内存（NVM）存储介质的持久内存优化实例。

本笔记本展示了如何使用与 `Tair` 向量数据库相关的功能。

您需要安装 `langchain-community`，使用 `pip install -qU langchain-community` 来使用此集成。

要运行，您需要有一个启动并运行的 `Tair` 实例。
