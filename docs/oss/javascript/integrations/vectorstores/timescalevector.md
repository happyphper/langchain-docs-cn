...
初始化 TimescaleVector 存储后，您可以将其用作 [检索器](/oss/javascript/langchain/retrieval)。
...
接下来，我们将定义一个元数据函数，用于从 JSON 记录中提取相关元数据。我们将把此函数传递给 JSONLoader。有关更多详细信息，请参阅 [JSON 文档加载器文档](/oss/javascript/integrations/document_loaders/json)。
