...
您的应用程序代码必须存放在 GitHub 仓库中，才能在 LangSmith 上部署。支持公共和私有仓库。对于本快速入门，请首先按照 [本地服务器设置指南](/oss/javascript/langgraph/studio#setup-local-agent-server) 确保您的应用程序与 LangGraph 兼容。然后，将您的代码推送到仓库。
