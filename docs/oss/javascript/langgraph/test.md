...
请注意，本指南是 LangGraph 特有的，涵盖了具有自定义结构的图（graph）相关场景——如果你是刚开始接触，请查看[此章节](/oss/javascript/langchain/test/)，该章节使用了 LangChain 内置的 <a href="https://reference.langchain.com/javascript/functions/langchain.index.createAgent.html" target="_blank" rel="noreferrer" class="link"><code>createAgent</code></a> 函数。
...
2.  调用智能体的 [`update_state`](/oss/javascript/langgraph/use-time-travel) 方法，并将 [`asNode`](/oss/javascript/langgraph/persistence#as-node) 参数设置为*你想要开始测试的节点之前*的那个节点的名称。
