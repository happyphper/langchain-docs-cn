...
**工作原理：**
- 将文件存储在运行时提供的 LangGraph <a href="https://reference.langchain.com/python/langsmith/deployment/remote_graph/" target="_blank" rel="noreferrer" class="link"><code>BaseStore</code></a> 中，实现跨线程的持久化存储。

**最适合：**
- 当你已经运行着一个配置好的 LangGraph 存储时（例如，Redis、Postgres 或 <a href="https://reference.langchain.com/python/langsmith/deployment/remote_graph/" target="_blank" rel="noreferrer" class="link"><code>BaseStore</code></a> 背后的云实现）。
...
agent = create_deep_agent(
    ...
    store=InMemoryStore()  # 存储传递给 create_deep_agent，而不是后端
)
```
