...
聊天提示由一个 <a href="https://reference.langchain.com/python/langchain/messages/#langchain.messages.SystemMessage" target="_blank" rel="noreferrer" class="link"><code>SystemMessage</code></a> 和多对包含样本（即问题/SQL 对）的 `HumanMessage`/`AIMessage` 组成。您可以向此列表追加样本对，但这并非旨在支持典型的自然语言对话。
