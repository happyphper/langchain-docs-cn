...
title: ChatMistralAI
---
这将帮助您开始使用 Mistral [聊天模型](/oss/python/langchain/models)。有关 `ChatMistralAI` 所有功能和配置的详细文档，请前往 [API 参考](https://python.langchain.com/api_reference/mistralai/chat_models/langchain_mistralai.chat_models.ChatMistralAI.html)。`ChatMistralAI` 类构建于 [Mistral API](https://docs.mistral.ai/api/) 之上。有关 Mistral 支持的所有模型列表，请查看 [此页面](https://docs.mistral.ai/getting-started/models/)。
