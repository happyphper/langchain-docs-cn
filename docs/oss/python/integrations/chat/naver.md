...
请注意，`AIMessage` 有一个 <a href="https://reference.langchain.com/python/langchain/messages/#langchain.messages.AIMessage.tool_calls" target="_blank" rel="noreferrer" class="link"><code>tool_calls</code></a> 属性。它包含一个标准化的 `ToolCall` 格式，该格式与模型提供商无关。
