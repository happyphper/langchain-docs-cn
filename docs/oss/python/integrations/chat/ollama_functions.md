...
<strong>LangChain Ollama 集成包已正式支持工具调用功能。[点击此处查看文档](/oss/python/integrations/chat/ollama#tools)。</strong>
...
<strong>这是一个实验性的包装器，试图为本身不支持工具调用的模型添加此功能。请谨慎使用。</strong>
...
const toolSystemPromptTemplate = <code v-pre>You have access to the following tools:
