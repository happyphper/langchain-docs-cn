...
title: ChatTongyi
---
通义千问（Tongyi Qwen）是由阿里巴巴达摩院开发的大语言模型。它能够通过自然语言理解和语义分析来理解用户意图，基于用户输入的自然语言，在不同领域和任务中为用户提供服务与帮助。通过提供清晰、详细的指令，您可以获得更符合预期的结果。

在本笔记本中，我们将主要介绍如何在 `Chat`（对应于 langchain 中的 `langchain/chat_models` 包）中使用 LangChain 与 [通义千问](https://www.aliyun.com/product/dashscope)。
...
ChatTongyi 支持工具调用 API，允许您描述工具及其参数，并让模型返回一个包含要调用的工具及其输入参数的 JSON 对象。
