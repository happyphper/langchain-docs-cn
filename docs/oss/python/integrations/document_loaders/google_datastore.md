---
title: Google Firestore（Datastore 模式）
---
...
本笔记本将介绍如何使用 [Datastore 模式下的 Firestore](https://cloud.google.com/datastore)，通过 `DatastoreLoader` 和 `DatastoreSaver` 来[保存、加载和删除 langchain 文档](/oss/python/integrations/document_loaders)。
...
1.  `source` - 加载文档的源。它可以是 Query 的实例，也可以是要读取的 Datastore kind 的名称。
