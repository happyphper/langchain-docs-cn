...
---
本指南提供了 `PyPDF` [文档加载器](https://python.langchain.com/docs/concepts/document_loaders) 的快速入门概览。有关所有 DocumentLoader 功能和配置的详细文档，请参阅 [API 参考](https://python.langchain.com/api_reference/community/document_loaders/langchain_community.document_loaders.pdf.PyPDFium2Loader.html)。
...
在此模式下，PDF 按页面分割，生成的 Document 元数据包含页码。但在某些情况下，我们可能希望将 PDF 作为单个文本流处理（这样就不会将某些段落切成两半）。在这种情况下，您可以使用 *single* 模式：
...
这可以简单地是 \n，或 \f 来清晰表示页面更改，或 \<!-- PAGE BREAK --> 以便在 Markdown 查看器中无缝注入而不产生视觉影响。
