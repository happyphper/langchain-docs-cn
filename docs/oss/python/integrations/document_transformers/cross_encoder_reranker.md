...
这建立在 ContextualCompressionRetriever 中的思想之上。本文档的整体结构源自 [Cohere 重排序器文档](/oss/python/integrations/retrievers/cohere-reranker)。
...
现在，让我们用 `ContextualCompressionRetriever` 包装我们的基础检索器。`CrossEncoderReranker` 使用 `HuggingFaceCrossEncoder` 对返回的结果进行重排序。
...
这是一个用于创建与 `SagemakerEndpointCrossEncoder` 兼容的端点的示例 `inference.py`。有关分步指导的更多详细信息，请参阅 [这篇文章](https://huggingface.co/blog/kchoe/deploy-any-huggingface-model-to-sagemaker)。
