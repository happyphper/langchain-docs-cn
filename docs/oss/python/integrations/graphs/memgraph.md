...
除了[设置部分](#setting-up)中的所有导入外，导入 `LLMGraphTransformer` 和 <a href="https://reference.langchain.com/python/langchain_core/documents/#langchain_core.documents.base.Document" target="_blank" rel="noreferrer" class="link"><code>Document</code></a>，它们将用于从文本中提取结构化信息。
