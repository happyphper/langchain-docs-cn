...
2.  更新您的导入语句，使用来自 `@langchain/openai` 包的新 `AzureOpenAI` 和 <a href="https://reference.langchain.com/python/integrations/langchain_openai/AzureChatOpenAI" target="_blank" rel="noreferrer" class="link"><code>AzureChatOpenAI</code></a> 类：

```typescript
import { AzureOpenAI } from "@langchain/openai";
```

3.  更新您的代码以使用新的 `AzureOpenAI` 和 <a href="https://reference.langchain.com/python/integrations/langchain_openai/AzureChatOpenAI" target="_blank" rel="noreferrer" class="link"><code>AzureChatOpenAI</code></a> 类，并传递所需的参数：
