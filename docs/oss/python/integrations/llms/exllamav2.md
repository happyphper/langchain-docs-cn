...
title: ExLlamaV2
---
[ExLlamav2](https://github.com/turboderp/exllamav2) 是一个用于在现代消费级 GPU 上本地运行 LLM 的快速推理库。
...
[TheBloke 的](https://huggingface.co/TheBloke) Hugging Face 模型有一个 `Provided files` 部分，其中列出了运行不同量化大小和方法的模型所需的内存（例如：[Mistral-7B-Instruct-v0.2-GPTQ](https://huggingface.co/TheBloke/Mistral-7B-Instruct-v0.2-GPTQ)）。
