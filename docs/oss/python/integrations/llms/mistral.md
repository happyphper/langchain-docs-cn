...
您当前正在查阅的是关于将 Mistral 模型用作文本补全模型的文档。Mistral 上提供的许多流行模型是[聊天补全模型](/oss/python/langchain/models)。

您可能正在寻找[这个页面](/oss/python/integrations/chat/mistral/)。

</Warning>

<Tip>

<strong>想在本地运行 Mistral 的模型？查看我们的 [Ollama 集成](/oss/python/integrations/chat/ollama)。</strong>
