...
title: Replicate
---
>[Replicate](https://replicate.com/blog/machine-learning-needs-better-tools) 在云端运行机器学习模型。我们拥有一个开源模型库，您只需几行代码即可运行。如果您正在构建自己的机器学习模型，Replicate 可以轻松地大规模部署它们。
...
```python
# 魔法命令，用于自动重新加载外部模块，以防您在处理此笔记本时修改了 langchain
%load_ext autoreload
%autoreload 2
```
...
在 [replicate 探索页面](https://replicate.com/explore) 上找到一个模型，然后以 `model_name/version` 格式粘贴模型名称和版本。
...
例如，如果我们运行 stable diffusion 并想要更改图像尺寸：
...
您可以选择在生成过程中流式传输响应，这对于向用户展示耗时生成的交互性很有帮助。有关更多信息，请参阅 [流式传输](/oss/python/langchain/streaming) 的详细文档。
...
您还可以指定停止序列。如果您有一个明确的停止序列用于您将要解析的生成，那么一旦达到一个或多个停止序列就取消生成，而不是让模型一直漫谈到指定的 `max_length`，这样更好（更便宜、更快！）。无论您是否处于流式模式，停止序列都有效，并且 Replicate 只向您收取达到停止序列之前的生成费用。
...
LangChain 的核心就是... 链式调用！这里有一个如何做到这一点的例子。

```python
from langchain_classic.chains import SimpleSequentialChain
```

首先，让我们将这个模型的 LLM 定义为 flan-5，将 text2image 定义为 stable diffusion 模型。
