...
> 是一个去中心化的开源开发者社区。他们开发的软件依据 Apache 许可证的条款分发，这是一个宽松的自由及开源软件（FOSS）许可证。Apache 项目的特点在于其基于协作和共识的开发过程，以及开放且务实的软件许可证。这意味着它允许免费获得软件的开发者，在非自由条款下重新分发该软件。每个项目都由一个自选的技术专家团队管理，他们都是项目的活跃贡献者。
...
>[Apache Doris](https://doris.apache.org/) 是一个用于实时分析的现代数据仓库。它能够对大规模实时数据进行闪电般快速的分析。
>
