...
要在 LangChain 中原生使用 Prem 模板，您需要在 `HumanMessage` 中传递一个 ID。此 ID 应是您提示模板变量的名称。<a href="https://reference.langchain.com/python/langchain/messages/#langchain.messages.HumanMessage" target="_blank" rel="noreferrer" class="link"><code>HumanMessage</code></a> 中的 `content` 应是该变量的值。
