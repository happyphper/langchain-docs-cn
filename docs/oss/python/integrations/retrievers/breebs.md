...
任何人都可以基于存储在 Google Drive 文件夹中的 PDF 文件创建一个 Breeb（知识胶囊）。
任何 LLM/聊天机器人 都可以使用 Breeb 来提升其专业知识、减少幻觉并提供信息来源。
在幕后，Breebs 实现了多种检索增强生成（RAG）模型，以便在每次交互中无缝提供有用的上下文。

## 可用的 Breebs 列表

要获取完整的 Breebs 列表，包括其密钥（`breeb_key`）和描述，请访问：
[breebs.promptbreeders.com/web/listbreebs](https://breebs.promptbreeders.com/web/listbreebs)。
社区已经创建了数十个 Breebs，并且可以免费使用。它们涵盖了广泛的专业领域，从有机化学到神话，以及关于搭讪技巧和去中心化金融的建议。

## 创建新的 Breeb

要生成一个新的 Breeb，只需将 PDF 文件整理到一个公开共享的 Google Drive 文件夹中，然后在 [BREEBS 网站](https://www.breebs.com/) 上点击“创建 Breeb”按钮启动创建流程。目前最多可以包含 120 个文件，总字符数限制为 1500 万。
