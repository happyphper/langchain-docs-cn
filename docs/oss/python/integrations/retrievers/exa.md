...
本指南将帮助您开始使用 Exa [检索器](/oss/python/langchain/retrieval)。有关 `ExaRetriever` 所有功能和配置的详细文档，请参阅 [API 参考](https://api.js.langchain.com/classes/langchain_exa.ExaRetriever.html)。
...
`Exa` 类在查找您的 API 密钥时默认使用 `EXASEARCH_API_KEY`。
...
"He has no idea what's coming. The ruble has already lost 30 percent of its value, the Russian stock market has lost 40 percent of its value, and
