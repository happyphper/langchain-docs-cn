...
`langchain-graph-retriever` 包中的 `GraphRetriever` 提供了一个 LangChain [检索器](/oss/python/langchain/retrieval/)，它结合了对向量的**非结构化**相似性搜索和对元数据属性的**结构化**遍历。这使得可以在**现有**向量存储之上进行基于图的检索。
...
```
关于 `ASTRA_DB_API_ENDPOINT` 和 `ASTRA_DB_APPLICATION_TOKEN` 凭据，请查阅 [AstraDB 向量存储指南](/oss/python/integrations/vectorstores/astradb)。
...
有关创建 Cassandra 连接的帮助，请查阅 [Apache Cassandra 向量存储指南](/oss/python/integrations/vectorstores/cassandra#connection-parameters)
...
有关创建 OpenSearch 连接的帮助，请查阅 [OpenSearch 向量存储指南](/oss/python/integrations/vectorstores/opensearch)。
...
有关创建 Chroma 连接的帮助，请查阅 [Chroma 向量存储指南](/oss/python/integrations/vectorstores/chroma)。
