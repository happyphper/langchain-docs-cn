...
我们可以将其用作一个[检索器](/oss/python/langchain/retrieval)。下文将展示此集成特有的功能。了解之后，探索[相关的用例页面](/oss/python/langchain/rag)可能会有所帮助，以学习如何将此向量存储作为更大链条的一部分来使用。
...
如果你想从单个查询中获得自动化追踪，也可以通过取消下面的注释来设置你的 [LangSmith](https://docs.langchain.com/langsmith/home) API 密钥：
...
有关 `TavilySearchAPIRetriever` 所有功能和配置的详细文档，请前往 [API 参考](https://python.langchain.com/api_reference/community/retrievers/langchain_community.retrievers.tavily_search_api.TavilySearchAPIRetriever.html)。
