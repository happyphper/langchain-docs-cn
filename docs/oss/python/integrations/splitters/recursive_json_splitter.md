...
title: 拆分 JSON 数据
---
这个 JSON 分割器在控制分块大小的同时，对 JSON 数据进行[分割](/oss/python/integrations/splitters/)。它采用深度优先的方式遍历 JSON 数据并构建更小的 JSON 块。它会尽量保持嵌套的 JSON 对象完整，但如果需要将块大小保持在 `min_chunk_size` 和 `max_chunk_size` 之间，也会对它们进行分割。
