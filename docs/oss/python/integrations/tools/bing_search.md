...
title: Bing 搜索
---
> [Bing 搜索](https://learn.microsoft.com/en-us/bing/search-apis/bing-web-search/) 是一项 Azure 服务，能够提供安全、无广告、基于位置感知的搜索结果，从数十亿网络文档中提取相关信息。通过利用 Bing 在一次 API 调用中梳理数十亿网页、图片、视频和新闻的能力，帮助您的用户从全球互联网中找到他们所需的内容。
...
我们在此展示如何将其用作[智能体](/oss/python/langchain/agents)的一部分。我们使用 OpenAI Functions Agent，因此需要为此设置和安装所需的依赖项。我们还将使用 [LangSmith Hub](https://smith.langchain.com/hub) 来拉取提示词，因此也需要安装它。
