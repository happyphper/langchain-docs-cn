...
---
与各类 DeFi 协议交互。非托管式。工具返回*未签名交易*。该工具包构建在通用 DeFi API ([Compass API](https://api.compasslabs.ai/)) 之上，允许代理执行以下金融操作：
