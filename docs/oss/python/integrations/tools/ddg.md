...
默认情况下，结果以原始搜索结果中键值对的逗号分隔字符串形式返回。您也可以通过设置 `output_format="list"` 将搜索结果作为列表返回，或通过设置 `output_format="json"` 将其作为 JSON 字符串返回。
...
您也可以直接将自定义的 `DuckDuckGoSearchAPIWrapper` 传递给 `DuckDuckGoSearchResults`，以便对搜索结果提供更多控制。
