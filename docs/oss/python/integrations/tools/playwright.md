...
---
>[Playwright](https://github.com/microsoft/playwright) 是由 `Microsoft` 开发的开源自动化工具，允许你以编程方式控制和自动化 Web 浏览器。它专为端到端测试、网页抓取以及跨多种 Web 浏览器（如 `Chromium`、`Firefox` 和 `WebKit`）的自动化任务而设计。

此工具包用于与浏览器交互。虽然其他工具（如 `Requests` 工具）适用于静态网站，但 `PlayWright Browser` 工具包能让你的智能体（agent）在网络上导航并与动态渲染的网站进行交互。

`PlayWright Browser` 工具包内捆绑的一些工具包括：

- `NavigateTool` (navigate_browser) - 导航到 URL
- `NavigateBackTool` (previous_page) - 等待元素出现
- `ClickTool` (click_element) - 点击元素（通过选择器指定）
...
始终建议使用 `from_browser` 方法进行实例化，以确保浏览器上下文被正确初始化和管理，从而实现无缝交互和资源优化。
...
浏览器工具中有几个是 `StructuredTool`，这意味着它们需要多个参数。这些工具与早于 `STRUCTURED_CHAT_ZERO_SHOT_REACT_DESCRIPTION` 的智能体不兼容（开箱即用）。
