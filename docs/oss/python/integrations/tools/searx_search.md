...
title: SearxNG 搜索
---
本笔记本将介绍如何使用自托管的 `SearxNG` 搜索 API 进行网络搜索。

您可以[查看此链接](https://docs.searxng.org/dev/search_api.html)以获取有关 `Searx API` 参数的更多信息。
...
对于某些搜索引擎，如果存在直接的 `answer`（答案），包装器将打印答案而不是完整的搜索结果列表。如果您想获取所有结果，可以使用包装器的 `results` 方法。
...
SearxNG 支持 [135 个搜索引擎](https://docs.searxng.org/user/configured_engines.html)。您还可以使用任意命名的参数自定义 Searx 包装器，这些参数将传递给 Searx 搜索 API。在下面的示例中，我们将更深入地利用 searx 搜索 API 的自定义搜索参数。
...
在此示例中，我们将使用 `categories` 参数查找科学论文，并将结果限制在特定的 `time_range` 内（并非所有引擎都支持时间范围选项）。
...
在此示例中，我们在 `it` 类别下查询 `large language models`。然后过滤出来自 GitHub 的结果。
...
'Hill, Fall 2022)',
  'title': 'llm-seminar',
  'link': 'https://github.com/craffel/llm
