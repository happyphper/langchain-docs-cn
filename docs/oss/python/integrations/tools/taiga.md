...
以下是一个更完整的示例，展示了如何在链或代理中将 `create_entity_tool` 和 `search_entities_tool` 工具与 LLM 集成。此示例假设您有一个函数（如 <a href="https://reference.langchain.com/python/langchain/agents/#langchain.agents.create_agent" target="_blank" rel="noreferrer" class="link"><code>create_agent</code></a>），可以设置一个能够在适当时调用工具的 LangChain 风格代理。
