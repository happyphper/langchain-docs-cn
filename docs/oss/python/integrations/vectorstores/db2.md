---
title: IBM Db2 向量存储与向量搜索
---
LangChain 的 Db2 集成 (langchain-db2) 为 IBM 关系型数据库 Db2 版本 v12.1.2 及以上提供了向量存储和向量搜索功能，该集成在 MIT 许可证下分发。用户可以直接使用提供的实现，也可以根据特定需求进行定制。
主要功能包括：
...
安装该包时，其依赖项如 `langchain-core` 和 `ibm_db` 也应一并安装。
...
以下示例代码将展示如何连接到 Db2 数据库。除了上述依赖项，您还需要一个正在运行的 Db2 数据库实例（版本 v12.1.2+，支持向量数据类型）。
...
（您可以手动连接到 Db2 数据库，将会看到三个表：Documents_DOT、Documents_COSINE 和 Documents_EUCLIDEAN。）
...
### 演示对向量存储进行高级搜索，包括使用和不使用属性过滤的情况

使用过滤时，我们只选择文档 id 为 101 的文档，其他都不选
