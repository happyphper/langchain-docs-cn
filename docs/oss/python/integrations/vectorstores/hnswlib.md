...
本指南提供了快速入门 HNSWLib [向量存储](/oss/python/integrations/vectorstores) 的概述。有关 `HNSWLib` 所有功能和配置的详细文档，请参阅 [API 参考](https://api.js.langchain.com/classes/langchain_community_vectorstores_hnswlib.HNSWLib.html)。
...
本指南还将使用 [OpenAI 嵌入](/oss/python/integrations/text_embedding/openai)，这需要你安装 `@langchain/openai` 集成包。如果你愿意，也可以使用 [其他支持的嵌入模型](/oss/python/integrations/text_embedding)。
...
你也可以将向量存储转换为 [检索器](/oss/python/langchain/retrieval)，以便在你的链中更轻松地使用。
