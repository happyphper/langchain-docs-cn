...
我们将使用 <a href="https://reference.langchain.com/python/langchain_core/vectorstores/?h=#langchain_core.vectorstores.base.VectorStore" target="_blank" rel="noreferrer" class="link"><code>VectorStore</code></a> 客户端连接到索引，该客户端实现了 `similarity_search` 函数以检索匹配的文档。
