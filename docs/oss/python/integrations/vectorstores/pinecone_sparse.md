...
我们还可以在搜索时返回 `(document, score)` 元组列表中的相似性分数。其中 `document` 是一个包含文本内容和元数据的 LangChain <a href="https://reference.langchain.com/python/langchain_core/documents/#langchain_core.documents.base.Document" target="_blank" rel="noreferrer" class="link"><code>Document</code></a> 对象。
