...
您可以使用[中间件](/oss/python/langchain/middleware)在关键节点拦截执行，从而实施防护栏——例如在智能体启动前、完成后，或在模型调用和工具调用前后。
...
有关实现审批工作流的完整详细信息，请参阅[人工介入文档](/oss/python/langchain/human-in-the-loop)。
...
- [中间件文档 (Middleware documentation)](/oss/python/langchain/middleware) - 自定义中间件的完整指南
- [中间件 API 参考 (Middleware API reference)](https://reference.langchain.com/python/langchain/middleware/) - 详细的 API 说明
- [人工介入 (Human-in-the-loop)](/oss/python/langchain/human-in-the-loop) - 为敏感操作添加人工审核
- [测试智能体 (Testing agents)](/oss/python/langchain/test) - 安全机制的测试策略
