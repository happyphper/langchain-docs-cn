...
当单个智能体拥有过多[工具](/oss/python/langchain/tools)且在选择使用哪个工具时决策不佳时，当任务需要具有大量上下文（长提示和领域特定工具）的专业知识时，或者当你需要强制执行顺序约束（即只有在满足特定条件后才能解锁某些能力）时，多智能体模式尤其有价值。

<Tip>

多智能体设计的核心是<strong>[上下文工程](/oss/python/langchain/context-engineering)</strong>——决定每个智能体能看到什么信息。系统的质量取决于能否确保每个智能体都能访问其任务所需的正确数据。
...
| [**交接**](/oss/python/langchain/multi-agent/handoffs) | 行为根据状态动态变化。工具调用会更新状态变量，从而触发路由或配置更改，切换智能体或调整当前智能体的工具和提示。 |
| [**技能**](/oss/python/langchain/multi-agent/skills) | 按需加载的专用提示和知识。单个智能体保持控制，同时根据需要从技能加载上下文。 |
| [**路由器**](/oss/python/langchain/multi-agent/router) | 路由步骤对输入进行分类，并将其定向到一个或多个专用智能体。结果被合成为一个组合响应。 |
...
- **处理的令牌数**：所有调用中总的[上下文窗口](/oss/python/langchain/context-engineering)使用量。令牌越多 = 处理成本越高，且可能触及上下文限制。
