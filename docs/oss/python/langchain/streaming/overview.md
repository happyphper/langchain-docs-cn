...
为了处理人机协同（human-in-the-loop）的[中断](/oss/python/langchain/human-in-the-loop)，我们在[上面的示例](#streaming-tool-calls)基础上进行构建：

1.  我们使用[人机协同中间件和一个检查点](/oss/python/langchain/human-in-the-loop#configuring-interrupts)来配置智能体（agent）。
2.  我们在 `"updates"` 流模式期间收集生成的中断。
3.  我们使用一个[命令](/oss/python/langchain/human-in-the-loop#responding-to-interrupts)来响应这些中断。
...
接下来，我们为每个中断收集一个[决策](/oss/python/langchain/human-in-the-loop#interrupt-decision-types)。重要的是，决策的顺序必须与我们收集到的操作顺序相匹配。
...
然后，我们可以通过向同一个流式循环传递一个[命令](/oss/python/langchain/human-in-the-loop#responding-to-interrupts)来恢复执行：
...
2.  我们为此 LLM 和外部的["监督者"](/oss/python/langchain/multi-agent) LLM 添加一个字符串标签。
...
- 使用[多智能体](/oss/python/langchain/multi-agent)系统时，控制哪些智能体流式传输其输出
...
- [前端流式传输](/oss/python/langchain/streaming/frontend) — 使用 `useStream` 构建 React UI，实现实时智能体交互
- [使用聊天模型进行流式传输](/oss/python/langchain/models#stream) — 无需使用智能体或图，直接从聊天模型流式传输令牌
- [人机协同（human-in-the-loop）流式传输](/oss/python/langchain/human-in-the-loop#streaming-with-hil) — 在处理人工审核中断的同时流式传输智能体进度
- [LangGraph 流式传输](/oss/python/langgraph/streaming) — 高级流式传输选项，包括 `values`、`debug` 模式以及子图流式传输
