...
删除消息时，<strong>请确保</strong>生成的消息历史记录是有效的。检查您使用的LLM提供商的限制。例如：
...
按照惯例，大多数数据库特定的库在检查点或存储实例上定义了一个`setup()`方法，用于运行所需的迁移。但是，您应该检查您特定的<a href="https://reference.langchain.com/python/langgraph/checkpoints/#langgraph.checkpoint.base.BaseCheckpointSaver" target="_blank" rel="noreferrer" class="link"><code>BaseCheckpointSaver</code></a>或<a href="https://reference.langchain.com/python/langsmith/deployment/remote_graph/" target="_blank" rel="noreferrer" class="link"><code>BaseStore</code></a>实现，以确认确切的方法名称和用法。
