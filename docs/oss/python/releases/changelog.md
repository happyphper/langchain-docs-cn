...
* [动态系统提示中间件](/oss/langchain/agents#dynamic-system-prompt)：`dynamicSystemPromptMiddleware` 的返回值现在完全是附加性的。当返回 <a href="https://reference.langchain.com/python/langchain/messages/#langchain.messages.SystemMessage" target="_blank" rel="noreferrer" class="link"><code>SystemMessage</code></a> 或 `string` 时，它们会与现有的系统消息合并，而不是替换它们，这使得组合多个修改提示的中间件变得更加容易。
