在本地构建 LangChain 智能体时，可视化智能体的内部运行情况、实时交互以及调试出现的问题会非常有帮助。**LangSmith Studio** 是一个免费的视觉界面，用于从本地机器开发和测试您的 LangChain 智能体。
...
Studio 需要一个 LangSmith API 密钥来连接您的本地智能体。在项目的根目录下创建一个 `.env` 文件，并添加您从 [LangSmith](https://smith.langchain.com/settings) 获取的 API 密钥。
...
![Studio UI 中的智能体视图](/oss/javascript/images/studio_create-agent.png)

</Frame>

当 Studio 连接到您的本地智能体后，您可以快速迭代优化智能体的行为。运行测试输入，检查完整的执行轨迹，包括提示、工具参数、返回值以及令牌/延迟指标。当出现问题时，Studio 会捕获异常及其周围的状态，帮助您了解发生了什么。

开发服务器支持热重载——在代码中更改提示或工具签名，Studio 会立即反映这些更改。您可以从任何步骤重新运行对话线程来测试您的更改，而无需从头开始。这个工作流程适用于从简单的单工具智能体到复杂的多节点图。
